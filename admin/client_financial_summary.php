<?php
date_default_timezone_set('Africa/Lagos');
// admin/client_financial_summary.php - Admin Client Financial Summary (AJAX Infinite Scroll)
session_start();

ini_set('memory_limit', '512M');
ini_set('max_execution_time', 120);

require_once '../includes/config.php';
$pdo = getDbConnection(); 

if (!isset($_SESSION['user_role']) || $_SESSION['user_role'] !== 'admin') {
    if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
        echo json_encode(['status' => 'error', 'message' => 'Unauthorized']);
        exit();
    }
    header('Location: ../index.php');
    exit();
}

// ==========================================
// AJAX API ENDPOINT
// ==========================================
if (isset($_GET['ajax']) && $_GET['ajax'] == '1') {
    header('Content-Type: application/json');

    $page = isset($_GET['page']) ? max(1, intval($_GET['page'])) : 1;
    $limit = 50; 
    $offset = ($page - 1) * $limit;

    $search = $_GET['search'] ?? '';
    $filter = $_GET['filter'] ?? 'all';
    $zone = $_GET['zone'] ?? 'all';

    // Base query components
    $from_joins = "
        FROM clients c
        LEFT JOIN branches b ON c.branch_id = b.id
        LEFT JOIN areas a ON b.area_id = a.id
        LEFT JOIN zones z ON a.zone_id = z.id
        LEFT JOIN users u ON c.officer_username = u.username
        LEFT JOIN (
            SELECT client_id, SUM(remaining_balance) as active_balance, SUM(principal) as original_principal
            FROM disbursements 
            WHERE remaining_balance > 0
            GROUP BY client_id
        ) l ON c.id = l.client_id
        LEFT JOIN saving_balances s ON c.id = s.client_id
    ";

    // Build WHERE clauses
    $where_clauses = ["(c.status = 'active' OR COALESCE(l.active_balance, 0) > 0)"];
    $params = [];

    // 1. Zone Filter
    if ($zone === 'unassigned') {
        $where_clauses[] = "z.id IS NULL";
    } elseif ($zone !== 'all') {
        $where_clauses[] = "z.id = ?";
        $params[] = $zone;
    }

    // 2. Tab Filter (All, Debtor, Saver, Risk)
    if ($filter === 'debtor') {
        $where_clauses[] = "COALESCE(l.active_balance, 0) > 0";
    } elseif ($filter === 'saver') {
        $where_clauses[] = "COALESCE(l.active_balance, 0) <= 0";
    } elseif ($filter === 'risk') {
        $where_clauses[] = "(COALESCE(s.balance, 0) - COALESCE(l.active_balance, 0)) < 0";
    }

    // 3. Search Filter
    if (!empty($search)) {
        $searchTerm = '%' . $search . '%';
        $where_clauses[] = "(c.name LIKE ? OR c.phone LIKE ? OR b.name LIKE ? OR a.name LIKE ? OR c.officer_username LIKE ? OR u.full_name LIKE ? OR c.`union` LIKE ?)";
        array_push($params, $searchTerm, $searchTerm, $searchTerm, $searchTerm, $searchTerm, $searchTerm, $searchTerm);
    }

    $where_sql = " WHERE " . implode(' AND ', $where_clauses);

    // --- Fetch Totals (Only on page 1 for performance) ---
    $response = ['status' => 'success', 'data' => [], 'has_more' => false];

    if ($page === 1) {
        $totals_query = "
            SELECT 
                z.id as zone_id, 
                COALESCE(z.name, 'Unassigned / Orphaned') as zone_name,
                COUNT(c.id) as total_clients,
                SUM(CASE WHEN COALESCE(l.active_balance, 0) > 0 THEN 1 ELSE 0 END) as total_debtors,
                SUM(COALESCE(s.balance, 0)) as total_savings,
                SUM(COALESCE(l.active_balance, 0)) as total_loans
            $from_joins
            $where_sql
            GROUP BY z.id, z.name
            ORDER BY z.name ASC
        ";
        
        $stmt_totals = $pdo->prepare($totals_query);
        $stmt_totals->execute($params);
        $zone_totals_raw = $stmt_totals->fetchAll(PDO::FETCH_ASSOC);

        $zone_totals = [];
        $grand_totals = ['savings' => 0, 'loans' => 0, 'net' => 0, 'clients' => 0, 'debtors' => 0];

        foreach ($zone_totals_raw as $zt) {
            $zid = $zt['zone_id'] ?: 'unassigned';
            $sav = floatval($zt['total_savings']);
            $lns = floatval($zt['total_loans']);
            
            $zone_totals[$zid] = [
                'name' => $zt['zone_name'],
                'count' => intval($zt['total_clients']),
                'savings' => $sav,
                'loans' => $lns,
                'net' => $sav - $lns
            ];

            $grand_totals['savings'] += $sav;
            $grand_totals['loans'] += $lns;
            $grand_totals['clients'] += intval($zt['total_clients']);
            $grand_totals['debtors'] += intval($zt['total_debtors']);
        }
        $grand_totals['net'] = $grand_totals['savings'] - $grand_totals['loans'];

        $response['zone_totals'] = $zone_totals;
        $response['grand_totals'] = $grand_totals;
    }

    // --- Fetch Paginated Clients ---
    $query = "
        SELECT 
            c.id, c.name, c.phone, c.`union`, c.status as client_status,
            b.name as branch_name, a.name as area_name, z.name as zone_name, z.id as zone_id,
            COALESCE(u.full_name, c.officer_username) as officer_name,
            COALESCE(s.balance, 0) as total_savings,
            COALESCE(l.active_balance, 0) as loan_balance,
            COALESCE(l.original_principal, 0) as loan_principal
        $from_joins
        $where_sql
        ORDER BY z.name ASC, a.name ASC, b.name ASC, c.name ASC
        LIMIT $limit OFFSET $offset
    ";

    $stmt = $pdo->prepare($query);
    $stmt->execute($params);

    $clients = [];
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $active_loan = floatval($row['loan_balance']); 
        $principal = floatval($row['loan_principal']);
        $savings = floatval($row['total_savings']);
        $net_position = $savings - $active_loan;
        $has_active_loan = ($active_loan > 0);
        
        $loan_progress = 0;
        if ($has_active_loan && $principal > 0) {
            $repaid = max(0, $principal - $active_loan);
            $loan_progress = min(100, round(($repaid / $principal) * 100));
        }

        $status_type = 'saver'; 
        if ($has_active_loan) $status_type = 'debtor';
        if ($net_position < 0) $status_type = 'risk';

        $clients[] = [
            'id' => $row['id'],
            'zone_id' => $row['zone_id'] ?: 'unassigned',
            'name' => $row['name'],
            'phone' => $row['phone'] ?? '',
            'officer' => explode(' ', trim($row['officer_name']))[0],
            'branch' => $row['branch_name'] ?: 'Unassigned Branch',
            'area' => $row['area_name'] ?: 'Unassigned Area',
            'client_status' => strtolower($row['client_status']),
            'savings' => $savings,
            'loan_balance' => $active_loan,
            'loan_progress' => $loan_progress,
            'net_position' => $net_position,
            'has_loan' => $has_active_loan,
            'status_type' => $status_type
        ];
    }

    $response['data'] = $clients;
    $response['has_more'] = count($clients) === $limit;

    echo json_encode($response);
    exit();
}


// ==========================================
// PAGE INITIALIZATION
// ==========================================
$user_id = $_SESSION['user_id'];
$base_path = '../';

// Fetch Admin Profile
$stmt_t = $pdo->prepare("SELECT profile_pic FROM users WHERE id = ?");
$stmt_t->execute([$user_id]);
$user_data = $stmt_t->fetch(PDO::FETCH_ASSOC);
$profile_pic = !empty($user_data['profile_pic']) ? $user_data['profile_pic'] : 'default_avatar.png';

// Fetch All Zones for Dropdown
$stmt_zones = $pdo->query("SELECT id, name FROM zones ORDER BY name ASC");
$all_zones = $stmt_zones->fetchAll(PDO::FETCH_ASSOC);

?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no, viewport-fit=cover">
    <meta name="theme-color" content="#3b82f6">
    <title>Global Financial Summary | CUPAD Admin</title>
    
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">    
    <script src="https://cdn.tailwindcss.com"></script>

    <script>
    tailwind.config = {
        darkMode: 'class',
        theme: {
            extend: {
                colors: {
                    primary: '#3b82f6', secondary: '#8b5cf6',
                    success: '#22c55e', warning: '#f59e0b', error: '#ef4444',
                    bg: { light: '#f0f2f5', dark: '#0f172a' }
                }
            }
        }
    }
    </script>

    <style>
        :root {
            --primary-color: #3b82f6; --secondary-color: #8b5cf6;
            --success-color: #22c55e; --warning-color: #f59e0b; --error-color: #ef4444;
            --bg-primary: #f0f2f5; --bg-secondary: #ffffff; --bg-card: #ffffff;
            --bg-header: rgba(255, 255, 255, 0.95);
            --text-primary: #1f2937; --text-secondary: #6b7280; --border-color: #e5e7eb;
            --shadow-sm: 0 1px 2px 0 rgb(0 0 0 / 0.05);
            --shadow-md: 0 4px 6px -1px rgb(0 0 0 / 0.1);
        }
        
        html.dark {
            --bg-primary: #0f172a; --bg-secondary: #1e293b; --bg-card: #1e293b;
            --bg-header: rgba(15, 23, 42, 0.95);
            --text-primary: #f1f5f9; --text-secondary: #94a3b8; --border-color: rgba(255, 255, 255, 0.08);
        }

        body { font-family: 'Inter', sans-serif; background-color: var(--bg-primary); color: var(--text-primary); padding-bottom: 90px; }

        /* HEADER */
        .main-header { background: var(--bg-header); backdrop-filter: blur(12px); border-bottom: 1px solid var(--border-color); position: sticky; top: 0; z-index: 50; padding-top: env(safe-area-inset-top); }
        .navbar { display: flex; justify-content: space-between; align-items: center; padding: 0.8rem 1rem; max-width: 1280px; margin: 0 auto; }
        .logo { font-weight: 700; font-size: 1.25rem; color: var(--primary-color); display: flex; align-items: center; gap: 0.5rem; text-decoration: none; }
        .logo img { height: 32px; width: auto; }
        .profile-btn { width: 36px; height: 36px; border-radius: 50%; overflow: hidden; border: 2px solid var(--primary-color); background: var(--bg-secondary); cursor: pointer; display: flex; align-items: center; justify-content: center; position: relative; }
        .profile-btn img { width: 100%; height: 100%; object-fit: cover; }

        /* DASHBOARD GRID */
        .dashboard-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 1rem; margin-bottom: 1.5rem; }
        .dashboard-card { border-radius: 1.5rem; padding: 1.5rem; position: relative; overflow: hidden; box-shadow: var(--shadow-md); color: white; }
        .card-gradient-blue { background: linear-gradient(135deg, #3b82f6 0%, #2563eb 100%); }
        .card-gradient-orange { background: linear-gradient(135deg, #f59e0b 0%, #d97706 100%); }
        .card-gradient-purple { background: linear-gradient(135deg, #8b5cf6 0%, #7c3aed 100%); }
        .card-bg-icon { position: absolute; right: -10px; bottom: -20px; font-size: 6rem; color: white; opacity: 0.1; transform: rotate(-10deg); pointer-events: none; }

        /* CONTROLS */
        .controls-wrapper { position: sticky; top: 72px; z-index: 40; background: var(--bg-primary); padding-bottom: 1rem; padding-top: 0.5rem; }
        .controls-inner { display: flex; flex-direction: column; gap: 0.8rem; }
        .search-row { display: flex; gap: 0.5rem; }
        
        .co-select {
            background: var(--bg-card); border: 1px solid var(--border-color); color: var(--text-primary); 
            border-radius: 1rem; padding: 0.6rem 2.5rem 0.6rem 1rem; font-size: 0.9rem; font-weight: 600; 
            outline: none; cursor: pointer; box-shadow: var(--shadow-sm); height: 100%; 
            appearance: none; background-image: url("data:image/svg+xml;charset=US-ASCII,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%22292.4%22%20height%3D%22292.4%22%3E%3Cpath%20fill%3D%22%236b7280%22%20d%3D%22M287%2069.4a17.6%2017.6%200%200%200-13-5.4H18.4c-5%200-9.3%201.8-12.9%205.4A17.6%2017.6%200%200%200%200%2082.2c0%205%201.8%209.3%205.4%2012.9l128%20127.9c3.6%203.6%207.8%205.4%2012.8%205.4s9.2-1.8%2012.8-5.4L287%2095c3.5-3.5%205.4-7.8%205.4-12.8%200-5-1.9-9.2-5.5-12.8z%22%2F%3E%3C%2Fsvg%3E"); 
            background-repeat: no-repeat; background-position: right 1rem top 50%; background-size: 0.65rem auto; 
        }

        .search-container { background: var(--bg-card); border-radius: 1rem; padding: 0.5rem; border: 1px solid var(--border-color); display: flex; align-items: center; flex: 1; }
        .search-input { width: 100%; padding: 0.5rem 0.5rem 0.5rem 2.5rem; border: none; background: transparent; color: var(--text-primary); font-size: 1rem; outline: none; }
        
        .view-toggles { display: flex; background: var(--bg-card); border: 1px solid var(--border-color); border-radius: 1rem; padding: 0.25rem; }
        .view-btn { padding: 0.5rem 1rem; border-radius: 0.75rem; color: var(--text-secondary); cursor: pointer; border: none; background: transparent; }
        .view-btn.active { background: var(--primary-color); color: white; }

        .filter-tabs { display: flex; gap: 0.5rem; overflow-x: auto; padding: 0.2rem 0; scrollbar-width: none; }
        .filter-chip { white-space: nowrap; padding: 0.4rem 1rem; border-radius: 2rem; font-size: 0.8rem; font-weight: 600; background: var(--bg-card); border: 1px solid var(--border-color); color: var(--text-secondary); cursor: pointer; transition: all 0.2s; }
        .filter-chip.active { background: var(--primary-color); color: white; border-color: var(--primary-color); }

        /* ZONE SECTIONS */
        .zone-section { margin-bottom: 2rem; background: var(--bg-primary); border-radius: 1rem; overflow: hidden; }
        .zone-header { display: flex; justify-content: space-between; align-items: center; padding: 1rem; background: var(--bg-card); border: 1px solid var(--border-color); border-radius: 1rem; cursor: pointer; user-select: none; }
        .zone-header.collapsed .fa-chevron-down { transform: rotate(-90deg); }
        .zone-header i { transition: transform 0.3s; }
        .zone-content { display: block; padding-top: 1rem; animation: slideDown 0.3s ease-out; }
        .zone-content.hidden { display: none; }

        /* ZONE STAT CARDS */
        .zone-stat-card { display: flex; align-items: center; gap: 0.75rem; padding: 1rem; border-radius: 0.75rem; }
        .stat-icon { width: 36px; height: 36px; border-radius: 0.5rem; display: flex; align-items: center; justify-content: center; color: white; font-size: 0.9rem; flex-shrink: 0; }
        .stat-content { display: flex; flex-direction: column; gap: 0.15rem; flex: 1; min-width: 0; }
        .stat-label { font-size: 0.65rem; font-weight: 700; text-transform: uppercase; color: var(--text-secondary); }
        .stat-value { font-size: 0.95rem; font-weight: 800; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }

        /* GRID VIEW STYLES */
        .client-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 1rem; }
        .client-card { background: var(--bg-card); border-radius: 1rem; padding: 1.25rem; border: 1px solid var(--border-color); box-shadow: var(--shadow-sm); display: flex; flex-direction: column; gap: 0.8rem; position: relative; cursor: pointer; text-decoration: none; color: inherit; border-left: 4px solid transparent; }
        .client-card:hover { transform: translateY(-2px); border-color: var(--primary-color); }
        .cc-header { display: flex; align-items: center; gap: 0.75rem; }
        .cc-avatar { width: 42px; height: 42px; border-radius: 50%; background: linear-gradient(135deg, var(--bg-primary), var(--border-color)); color: var(--text-primary); display: flex; align-items: center; justify-content: center; font-weight: 700; flex-shrink: 0; }
        .cc-name { font-weight: 700; font-size: 0.95rem; color: var(--text-primary); line-height: 1.2; }
        .cc-stats { display: flex; justify-content: space-between; background: var(--bg-primary); padding: 0.6rem; border-radius: 0.75rem; border: 1px solid var(--border-color); }
        .stat-box { text-align: center; flex: 1; }
        .stat-box:first-child { border-right: 1px solid var(--border-color); }
        .net-pos-badge { position: absolute; top: 1rem; right: 1rem; font-size: 0.7rem; font-weight: 800; padding: 2px 8px; border-radius: 6px; }

        /* TABLE VIEW STYLES */
        .client-table-container { overflow-x: auto; border-radius: 1rem; border: 1px solid var(--border-color); background: var(--bg-card); }
        .client-table { border-collapse: collapse; width: 100%; font-size: 0.85rem; }
        .client-table th { background: var(--bg-primary); color: var(--text-secondary); text-align: left; padding: 1rem; font-weight: 600; text-transform: uppercase; font-size: 0.7rem; border-bottom: 1px solid var(--border-color); white-space: nowrap; }
        .client-table td { padding: 0.8rem 1rem; border-bottom: 1px solid var(--border-color); color: var(--text-primary); vertical-align: middle; white-space: nowrap; }
        .client-table tr:hover { background: var(--bg-primary); cursor: pointer; }
        .table-avatar { width: 30px; height: 30px; border-radius: 50%; background: var(--bg-primary); display: flex; align-items: center; justify-content: center; font-size: 0.8rem; font-weight: bold; border: 1px solid var(--border-color); margin-right: 0.75rem; }

        /* Status Colors */
        .status-saver { border-left-color: var(--success-color); }
        .status-debtor { border-left-color: var(--warning-color); }
        .status-risk { border-left-color: var(--error-color); }
        .pos-safe { background: rgba(34, 197, 94, 0.1); color: var(--success-color); }
        .pos-risk { background: rgba(239, 68, 68, 0.1); color: var(--error-color); }

        /* SKELETON LOADER */
        .skeleton { background: #cbd5e1; animation: pulse 1.5s infinite ease-in-out; border-radius: 0.5rem; color: transparent !important; }
        html.dark .skeleton { background: #334155; }
        @keyframes pulse { 0% { opacity: 1; } 50% { opacity: 0.4; } 100% { opacity: 1; } }

        /* Global View Mode Control (applied to body to control display of grid vs table) */
        body[data-view="grid"] .view-table { display: none !important; }
        body[data-view="table"] .view-grid { display: none !important; }

        /* MOBILE NAV */
        .mobile-bottom-nav { display: none; position: fixed; bottom: 0; left: 0; right: 0; background: var(--bg-secondary); border-top: 1px solid var(--border-color); padding: 0.5rem 1rem calc(0.5rem + env(safe-area-inset-bottom)); z-index: 100; justify-content: space-between; }
        .nav-item { display: flex; flex-direction: column; align-items: center; justify-content: center; text-decoration: none; color: var(--text-secondary); font-size: 0.7rem; font-weight: 500; gap: 4px; flex: 1; }
        .nav-item i { font-size: 1.4rem; }
        .nav-item.active { color: var(--primary-color); }

        @media (max-width: 768px) {
            .mobile-bottom-nav { display: flex; }
            .controls-wrapper { top: 60px; }
        }
        @keyframes slideDown { from { opacity: 0; transform: translateY(-10px); } to { opacity: 1; transform: translateY(0); } }
    </style>
</head>

<body data-view="grid">

    <!-- HEADER -->
    <header class="main-header">
        <nav class="navbar">
            <a href="dashboard.php" class="logo">
                <img src="<?php echo htmlspecialchars($base_path . 'favicon.ico'); ?>" alt="Logo">
                <span>CUPAD Admin</span>
            </a>
            
            <div class="flex items-center gap-3">
                <button id="theme-toggle" class="p-2 text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-full transition">
                    <i class="fas fa-moon dark:hidden"></i><i class="fas fa-sun hidden dark:inline"></i>
                </button>
                <div class="profile-btn" onclick="window.location.href='profile.php'">
                    <i class="fas fa-user text-gray-400"></i>
                    <?php if ($profile_pic !== 'default_avatar.png'): ?>
                        <img src="<?php echo $base_path . $profile_pic; ?>" alt="Profile" class="absolute inset-0" onerror="this.style.display='none'"> 
                    <?php endif; ?>
                </div>
            </div>
        </nav>
    </header>

    <main class="max-w-7xl mx-auto px-4 py-6">
        
        <div class="flex justify-between items-end mb-4">
            <div>
                <h1 class="text-2xl font-extrabold text-gray-900 dark:text-white" id="pageTitle">Global Summary</h1>
                <p class="text-xs text-gray-500 mt-1">Administrator • <?php echo date('M d, Y'); ?> • <span id="lblTotalClients">0</span> Clients</p>
            </div>
        </div>
        
        <!-- DASHBOARD STATS -->
        <div class="dashboard-grid">
            <div class="dashboard-card card-gradient-blue">
                <i class="fas fa-piggy-bank card-bg-icon"></i>
                <div class="relative z-10">
                    <div class="text-sm font-semibold uppercase opacity-90">Total Savings</div>
                    <div class="text-3xl font-extrabold mt-1 skeleton inline-block w-40 h-8" id="lblTotalSavings">₦0</div>
                </div>
            </div>

            <div class="dashboard-card card-gradient-orange">
                <i class="fas fa-hand-holding-usd card-bg-icon"></i>
                <div class="relative z-10">
                    <div class="text-sm font-semibold uppercase opacity-90">Outstanding Loans</div>
                    <div class="text-3xl font-extrabold mt-1 skeleton inline-block w-40 h-8" id="lblTotalLoans">₦0</div>
                </div>
            </div>

            <div class="dashboard-card card-gradient-purple">
                <i class="fas fa-chart-pie card-bg-icon"></i>
                <div class="relative z-10">
                    <div class="text-sm font-semibold uppercase opacity-90">Net Position</div>
                    <div class="text-3xl font-extrabold mt-1 skeleton inline-block w-40 h-8" id="lblTotalNet">₦0</div>
                </div>
            </div>
        </div>

        <!-- CONTROLS -->
        <div class="controls-wrapper">
            <div class="controls-inner">
                <div class="search-row flex-col md:flex-row w-full gap-2">
                    
                    <select id="zoneSelect" class="co-select w-full md:w-56">
                        <option value="all">Global (All Zones)</option>
                        <?php foreach($all_zones as $z): ?>
                            <option value="<?php echo htmlspecialchars($z['id']); ?>"><?php echo htmlspecialchars($z['name']); ?></option>
                        <?php endforeach; ?>
                        <option value="unassigned">Unassigned / Orphaned</option>
                    </select>

                    <div class="search-container flex-1 w-full relative">
                        <i class="fas fa-search text-gray-400 absolute left-4 top-1/2 transform -translate-y-1/2"></i>
                        <input type="text" id="searchInput" class="search-input" placeholder="Search name, phone, branch, union...">
                    </div>
                    
                    <div class="view-toggles shrink-0 self-end md:self-auto">
                        <button class="view-btn" id="btnGrid" title="Grid View"><i class="fas fa-th-large"></i></button>
                        <button class="view-btn" id="btnTable" title="List View"><i class="fas fa-list"></i></button>
                    </div>
                </div>

                <div class="filter-tabs" id="filterTabs">
                    <button class="filter-chip active" data-filter="all">All Clients</button>
                    <button class="filter-chip" data-filter="debtor">With Loans</button>
                    <button class="filter-chip" data-filter="saver">Debt Free</button>
                    <button class="filter-chip" data-filter="risk">High Risk</button>
                </div>
            </div>
        </div>

        <!-- ZONE LIST CONTAINER -->
        <div id="zonesContainer"></div>

        <!-- SKELETON LOADER -->
        <div id="skeletonLoader" class="hidden mt-6">
            <div class="client-grid">
                <?php for($i=0; $i<8; $i++): ?>
                <div class="client-card">
                    <div class="cc-header">
                        <div class="cc-avatar skeleton w-10 h-10"></div>
                        <div class="flex-1">
                            <div class="skeleton h-4 w-3/4 mb-2"></div>
                            <div class="skeleton h-3 w-1/2"></div>
                        </div>
                    </div>
                    <div class="skeleton h-16 w-full rounded"></div>
                </div>
                <?php endfor; ?>
            </div>
        </div>

        <!-- EMPTY STATE -->
        <div id="noResults" class="hidden flex flex-col items-center justify-center py-16 text-gray-400">
            <div class="w-20 h-20 bg-gray-100 dark:bg-gray-800 rounded-full flex items-center justify-center mb-4">
                <i class="fas fa-search text-3xl"></i>
            </div>
            <p class="text-lg font-semibold">No clients found</p>
            <p class="text-sm">Try adjusting your search or filters.</p>
        </div>

        <div id="scrollSentinel" class="h-10"></div>

    </main>

    <!-- MOBILE NAV -->
    <nav class="mobile-bottom-nav">
        <a href="dashboard.php" class="nav-item"><i class="fas fa-home"></i><span>Home</span></a>
        <a href="zones.php" class="nav-item"><i class="fas fa-layer-group"></i><span>Zones</span></a>
        <a href="users.php" class="nav-item"><i class="fas fa-users-cog"></i><span>Users</span></a>
        <a href="client_financial_summary.php" class="nav-item active">
            <div style="background: var(--primary-color); width: 45px; height: 45px; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: white; margin-top: -25px; box-shadow: 0 4px 10px rgba(59, 130, 246, 0.4);">
                <i class="fas fa-file-invoice-dollar" style="font-size: 1.2rem; margin:0;"></i>
             </div>
             <span style="font-size:0.65rem; font-weight:600; margin-top:2px;">Summary</span>
        </a>
    </nav>

    <script>
        // --- State Variables ---
        let currentPage = 1;
        let isLoading = false;
        let hasMoreData = true;
        let currentSearch = '';
        let currentFilter = 'all';
        let currentZone = 'all';
        let zoneTotalsCache = {};

        // --- DOM Elements ---
        const zonesContainer = document.getElementById('zonesContainer');
        const skeletonLoader = document.getElementById('skeletonLoader');
        const noResults = document.getElementById('noResults');
        const scrollSentinel = document.getElementById('scrollSentinel');
        const searchInput = document.getElementById('searchInput');
        const zoneSelect = document.getElementById('zoneSelect');
        const filterChips = document.querySelectorAll('.filter-chip');
        const btnGrid = document.getElementById('btnGrid');
        const btnTable = document.getElementById('btnTable');
        const bodyEl = document.body;

        // --- Format Helpers ---
        const formatMoney = (amount) => {
            return '₦' + parseFloat(amount).toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 });
        };

        const initThemeAndView = () => {
            if(localStorage.getItem('theme') === 'dark') document.documentElement.classList.add('dark');
            document.getElementById('theme-toggle').addEventListener('click', () => {
                document.documentElement.classList.toggle('dark');
                localStorage.setItem('theme', document.documentElement.classList.contains('dark') ? 'dark' : 'light');
            });

            const savedView = localStorage.getItem('adminClientViewPref') || 'grid';
            bodyEl.setAttribute('data-view', savedView);
            btnGrid.classList.toggle('active', savedView === 'grid');
            btnTable.classList.toggle('active', savedView === 'table');

            btnGrid.onclick = () => setView('grid');
            btnTable.onclick = () => setView('table');
        };

        const setView = (view) => {
            bodyEl.setAttribute('data-view', view);
            btnGrid.classList.toggle('active', view === 'grid');
            btnTable.classList.toggle('active', view === 'table');
            localStorage.setItem('adminClientViewPref', view);
        };

        // --- Component Generators ---
        const generateZoneHTML = (zoneId, zoneName, zTotals) => {
            const netColor = zTotals.net >= 0 ? 'blue' : 'orange';
            const netIcon = zTotals.net >= 0 ? 'chart-line' : 'exclamation-triangle';
            
            return `
            <div class="zone-section" id="zone-${zoneId}">
                <div class="zone-header" onclick="this.classList.toggle('collapsed'); this.nextElementSibling.classList.toggle('hidden');">
                    <div class="flex items-center gap-2">
                        <div class="w-8 h-8 rounded-full bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-300 flex items-center justify-center font-bold">
                            <i class="fas fa-layer-group text-sm"></i>
                        </div>
                        <div>
                            <div class="font-bold text-gray-800 dark:text-gray-100">${zoneName}</div>
                            <div class="text-xs text-gray-500">${zTotals.count} Members</div>
                        </div>
                    </div>
                    <i class="fas fa-chevron-down text-gray-400"></i>
                </div>
                <div class="zone-content">
                    <div class="px-4 pb-3">
                        <div class="grid grid-cols-3 gap-3">
                            <div class="zone-stat-card bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 border border-green-200 dark:border-green-800">
                                <div class="stat-icon bg-green-500"><i class="fas fa-piggy-bank"></i></div>
                                <div class="stat-content">
                                    <span class="stat-label">Savings</span>
                                    <span class="stat-value text-green-600 dark:text-green-400">${formatMoney(zTotals.savings)}</span>
                                </div>
                            </div>
                            <div class="zone-stat-card bg-gradient-to-br from-red-50 to-rose-50 dark:from-red-900/20 dark:to-rose-900/20 border border-red-200 dark:border-red-800">
                                <div class="stat-icon bg-red-500"><i class="fas fa-hand-holding-usd"></i></div>
                                <div class="stat-content">
                                    <span class="stat-label">Loans</span>
                                    <span class="stat-value text-red-600 dark:text-red-400">${formatMoney(zTotals.loans)}</span>
                                </div>
                            </div>
                            <div class="zone-stat-card bg-gradient-to-br from-${netColor}-50 to-indigo-50 dark:from-${netColor}-900/20 dark:to-indigo-900/20 border border-${netColor}-200 dark:border-${netColor}-800">
                                <div class="stat-icon bg-${netColor}-500"><i class="fas fa-${netIcon}"></i></div>
                                <div class="stat-content">
                                    <span class="stat-label">Net</span>
                                    <span class="stat-value text-${netColor}-600 dark:text-${netColor}-400">${formatMoney(zTotals.net)}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="view-grid client-grid px-4 pb-4"></div>
                    <div class="view-table px-4 pb-4">
                        <div class="client-table-container">
                            <table class="client-table">
                                <thead><tr><th>Client</th><th>Area/Branch/Officer</th><th>Savings</th><th>Loan Bal</th><th>Net Pos</th><th>Status</th></tr></thead>
                                <tbody></tbody>
                            </table>
                        </div>
                    </div>
                </div>
            </div>`;
        };

        const generateClientGridCard = (c) => {
            const initial = c.name.charAt(0).toUpperCase();
            const netClass = c.net_position >= 0 ? 'pos-safe' : 'pos-risk';
            const inactiveTag = c.client_status !== 'active' ? `<span class="absolute top-1.5 left-1.5 text-[0.6rem] font-bold bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300 px-2 py-0.5 rounded">INACTIVE</span>` : '';
            
            let progressHTML = c.has_loan ? `
                <div class="mt-1">
                    <div class="flex justify-between text-[0.65rem] text-gray-500 mb-1 font-medium"><span>Repayment Progress</span><span>${c.loan_progress}%</span></div>
                    <div class="w-full h-1.5 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden"><div class="h-full bg-gradient-to-r from-orange-500 to-red-500 rounded-full" style="width: ${c.loan_progress}%;"></div></div>
                </div>` : `<div class="mt-1 text-[0.65rem] text-gray-400 text-center py-1"><i class="fas fa-check-circle text-green-500 mr-1"></i> In Good Standing</div>`;

            return `
            <a href="history.php?client_id=${c.id}" class="client-card status-${c.status_type} ${c.client_status !== 'active' ? 'opacity-75' : ''}">
                ${inactiveTag}
                <span class="net-pos-badge ${netClass}">${c.net_position >= 0 ? '+' : ''}${formatMoney(c.net_position)}</span>
                <div class="cc-header">
                    <div class="cc-avatar">${initial}</div>
                    <div class="flex-1 min-w-0">
                        <div class="cc-name truncate">${c.name}</div>
                        <div class="text-xs text-blue-500 mt-0.5 font-medium flex items-center gap-1 truncate">
                            <i class="fas fa-building"></i> ${c.branch} <span class="mx-1">•</span> <i class="fas fa-user-tie"></i> ${c.officer}
                        </div>
                    </div>
                </div>
                <div class="cc-stats">
                    <div class="stat-box"><div class="stat-label">Saved</div><div class="stat-val text-green-500">${formatMoney(c.savings)}</div></div>
                    <div class="stat-box"><div class="stat-label">Loan</div><div class="stat-val ${c.loan_balance > 0 ? 'text-red-500' : 'text-gray-400'}">${formatMoney(c.loan_balance)}</div></div>
                </div>
                ${progressHTML}
            </a>`;
        };

        const generateClientTableRow = (c) => {
            const initial = c.name.charAt(0).toUpperCase();
            const inactiveTag = c.client_status !== 'active' ? `<span class="text-[0.6rem] bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300 px-1.5 py-0.5 rounded ml-2">INACTIVE</span>` : '';
            const statusBadge = c.has_loan ? `<span class="text-xs bg-red-100 text-red-600 px-2 py-1 rounded font-bold">Loan Active</span>` : `<span class="text-xs bg-green-100 text-green-600 px-2 py-1 rounded font-bold">Saver</span>`;
            
            return `
            <tr onclick="window.location.href='history.php?client_id=${c.id}'" class="${c.client_status !== 'active' ? 'opacity-70' : ''}">
                <td>
                    <div class="flex items-center">
                        <div class="table-avatar">${initial}</div>
                        <div>
                            <div class="font-bold flex items-center">${c.name} ${inactiveTag}</div>
                            <div class="text-xs text-gray-500">${c.phone}</div>
                        </div>
                    </div>
                </td>
                <td>
                    <div class="flex flex-col gap-1">
                        <span class="text-[10px] uppercase font-bold text-gray-500">${c.area} - ${c.branch}</span>
                        <span class="text-xs text-blue-600 dark:text-blue-400 font-semibold"><i class="fas fa-user-tie mr-1"></i>${c.officer}</span>
                    </div>
                </td>
                <td class="font-mono text-green-500 font-bold">${formatMoney(c.savings)}</td>
                <td class="font-mono text-red-500 font-bold">${formatMoney(c.loan_balance)}</td>
                <td class="font-mono font-bold ${c.net_position >= 0 ? 'text-blue-500' : 'text-orange-500'}">${formatMoney(c.net_position)}</td>
                <td>${statusBadge}</td>
            </tr>`;
        };

        // --- Data Fetching ---
        const fetchClients = async (reset = false) => {
            if (isLoading || (!hasMoreData && !reset)) return;
            
            isLoading = true;
            if (reset) {
                currentPage = 1;
                hasMoreData = true;
                zonesContainer.innerHTML = '';
                noResults.classList.add('hidden');
            }
            
            skeletonLoader.classList.remove('hidden');

            try {
                const params = new URLSearchParams({
                    ajax: 1,
                    page: currentPage,
                    search: currentSearch,
                    filter: currentFilter,
                    zone: currentZone
                });

                const response = await fetch(`?${params.toString()}`, {
                    headers: { 'X-Requested-With': 'XMLHttpRequest' }
                });
                
                const res = await response.json();
                
                if (res.status === 'success') {
                    if (currentPage === 1 && res.zone_totals) {
                        zoneTotalsCache = res.zone_totals;
                        updateGrandTotals(res.grand_totals);
                    }

                    if (reset && res.data.length === 0) {
                        noResults.classList.remove('hidden');
                    } else {
                        renderData(res.data);
                    }

                    hasMoreData = res.has_more;
                    currentPage++;
                }
            } catch (error) {
                console.error("Failed to fetch clients:", error);
            } finally {
                isLoading = false;
                skeletonLoader.classList.add('hidden');
            }
        };

        const renderData = (clients) => {
            clients.forEach(c => {
                let zoneWrapper = document.getElementById('zone-' + c.zone_id);
                
                if (!zoneWrapper) {
                    const zTotals = zoneTotalsCache[c.zone_id] || { count: 0, savings: 0, loans: 0, net: 0, name: 'Unknown Zone' };
                    const zoneName = zTotals.name;
                    zonesContainer.insertAdjacentHTML('beforeend', generateZoneHTML(c.zone_id, zoneName, zTotals));
                    zoneWrapper = document.getElementById('zone-' + c.zone_id);
                }

                const gridContainer = zoneWrapper.querySelector('.client-grid');
                const tableBody = zoneWrapper.querySelector('tbody');

                gridContainer.insertAdjacentHTML('beforeend', generateClientGridCard(c));
                tableBody.insertAdjacentHTML('beforeend', generateClientTableRow(c));
            });
        };

        const updateGrandTotals = (totals) => {
            document.getElementById('lblTotalClients').innerText = totals.clients.toLocaleString();
            
            const elSav = document.getElementById('lblTotalSavings');
            const elLoan = document.getElementById('lblTotalLoans');
            const elNet = document.getElementById('lblTotalNet');
            
            [elSav, elLoan, elNet].forEach(el => el.classList.remove('skeleton', 'text-transparent'));
            
            elSav.innerText = formatMoney(totals.savings);
            elLoan.innerText = formatMoney(totals.loans);
            elNet.innerText = formatMoney(totals.net);
        };

        // --- Event Listeners ---
        
        // Search Debounce
        let searchTimeout;
        searchInput.addEventListener('input', (e) => {
            clearTimeout(searchTimeout);
            searchTimeout = setTimeout(() => {
                currentSearch = e.target.value.trim();
                fetchClients(true);
            }, 400);
        });

        // Zone Filter
        zoneSelect.addEventListener('change', (e) => {
            currentZone = e.target.value;
            const zoneName = e.target.options[e.target.selectedIndex].text;
            document.getElementById('pageTitle').innerText = currentZone === 'all' ? 'Global Summary' : zoneName + ' Summary';
            fetchClients(true);
        });

        // Tabs Filter
        filterChips.forEach(chip => {
            chip.addEventListener('click', (e) => {
                filterChips.forEach(c => c.classList.remove('active'));
                e.target.classList.add('active');
                currentFilter = e.target.dataset.filter;
                fetchClients(true);
            });
        });

        // Intersection Observer for Infinite Scroll
        const observer = new IntersectionObserver((entries) => {
            if (entries[0].isIntersecting && !isLoading && hasMoreData) {
                fetchClients(false);
            }
        }, { rootMargin: '200px' });
        
        observer.observe(scrollSentinel);

        // Initialize
        initThemeAndView();
        fetchClients(true);

    </script>
</body>
</html>