...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax'])) {
    if (ob_get_length()) ob_clean(); // Prevent warnings from breaking JSON response
    ...
    try {
        switch ($action) {
            case 'create':
            case 'update':
                $id = !empty($_POST['id']) ? trim($_POST['id']) : 'p_' . time() . rand(1000, 9999);
                ...
                $auto_repayment = isset($_POST['auto_repayment']) ? 1 : 0;
                ...
                if ($savings_requirement_percentage < 0 || $savings_requirement_percentage > 100) throw new Exception("Savings requirement must be between 0 and 100");
                ...
                // Convert percentages into stored decimal form (e.g. 15% -> 0.15)
                $rate = $rate / 100;
                $savings_requirement_percentage = $savings_requirement_percentage / 100;
                ...
                if ($action === 'create') {
                    ...
                    if (!$stmt->execute([
                        ...
                        $savings_requirement_percentage, $disbursement_date_readonly, $allow_weekends, $grace_period_days, $auto_repayment
                    ])) {
                        throw new Exception("Failed to create loan plan in database.");
                    }
                } else {
                    ...
                    if (!$stmt->execute([
                        ...
                        $max_first_loan_amount, $disbursement_percentage, $registration_amount,
                        $min_days_after_registration, $min_waiting_days_full_term, $min_waiting_days_early_payoff,
                        $savings_requirement_percentage, $disbursement_date_readonly, $allow_weekends, $grace_period_days, $auto_repayment, $id
                    ])) {
                         throw new Exception("Failed to update loan plan in database.");
                    }
                }
            ...
            case 'delete':
                ...
                // Check if plan is in use safely
                $is_in_use = false;
                try {
                    ...
                    // Ignore if missing loans table (during setup/testing)
                }
            ...
            case 'get':
                ...
                // Present percentages correctly to the frontend JS formatting
                $plan['rate'] = floatval($plan['rate']) * 100;
                $plan['savings_requirement_percentage'] = floatval($plan['savings_requirement_percentage']) * 100;
    ...
    <style>
        ...
        * { box-sizing: border-box; outline: none; }
        ...
        #planForm {
            ...
            flex-direction: column;
            flex: 1;
            overflow: hidden; /* Prevents form from expanding beyond modal */
        }
        .modal-body {
            padding: 1.5rem;
            overflow-y: auto;
        ...
        .modal-footer {
            ...
            gap: 0.75rem;
            background: var(--bg-body);
            flex-shrink: 0; /* Ensure footer never shrinks */
        }
        ...
        @media (max-width: 768px) {
            ...
            .modal {
                max-height: 90vh; /* Better fit for mobile browsers */
                margin: 1rem;
                width: auto;
            }
            .modal-footer {
                    ...
                    <div class="plan-card" data-id="<?php echo htmlspecialchars($plan['id']); ?>">
                        <div class="plan-header">
                            ...
                            <div class="plan-rate">
                                ...
                                <?php echo ($plan['rate'] * 100); ?>% Interest
                            </div>
                        </div>
                        ...
                        <div class="plan-body">
                            ...
                            <div class="plan-badges">
                                <?php if ($plan['auto_repayment']): ?>
                                    <span class="badge badge-success"><i class="fas fa-robot"></i> Auto Repay</span>
                                <?php endif; ?>
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    <input type="number" name="savings_requirement_percentage" id="savingsReq" step="0.01" min="0" max="100" value="20">
                                    <span class="input-suffix">%</span>
                                </div>
                            </div>
                            <div class="form-group">
                    ...
                    <div class="form-section">
                        ...
                        <div class="toggle-row">
                            <label class="toggle-item">
                                <input type="checkbox" name="allow_weekends" id="allowWeekends" onchange="this.parentElement.classList.toggle('active', this.checked)">
                            ...
                            <label class="toggle-item">
                                <input type="checkbox" name="disbursement_date_readonly" id="dateReadonly" onchange="this.parentElement.classList.toggle('active', this.checked)" checked>
                                <div class="toggle-icon">
                             ...
                             <label class="toggle-item">
                                <input type="checkbox" name="auto_repayment" id="autoRepayment" onchange="this.parentElement.classList.toggle('active', this.checked)">
                                <div class="toggle-icon">
                                    <i class="fas fa-robot"></i>
                                </div>
                                <div class="toggle-text">
                                    <h4>Auto Repayment</h4>
                                    <p>Enable automatic repayment</p>
                                </div>
                            </label>
                        </div>
                    </div>
    ...
    <script>
        ...
        function openModal(planId = null) {
            ...
            if (planId) {
                ...
                // Set Defaults
                let aw = document.getElementById('allowWeekends');
                aw.checked = false;
                aw.parentElement.classList.remove('active');
                
                let dr = document.getElementById('dateReadonly');
                dr.checked = true;
                dr.parentElement.classList.add('active');
                
                let ar = document.getElementById('autoRepayment');
                ar.checked = false;
                ar.parentElement.classList.remove('active');
            }
        ...
        async function loadPlanData(planId) {
            try {
                ...
                if (result.success) {
                    ...
                    document.getElementById('rate').value = data.rate; // Provided dynamically *100 from backend
                    ...
                    document.getElementById('savingsReq').value = data.savings_requirement_percentage; // Dynamically *100 
                    document.getElementById('gracePeriod').value = data.grace_period_days;
                    
                    // Assign checkbox correctly and format toggle parent
                    let aw = document.getElementById('allowWeekends');
                    aw.checked = data.allow_weekends == 1;
                    aw.parentElement.classList.toggle('active', aw.checked);
                    
                    let dr = document.getElementById('dateReadonly');
                    dr.checked = data.disbursement_date_readonly == 1;
                    dr.parentElement.classList.toggle('active', dr.checked);
                    
                    let ar = document.getElementById('autoRepayment');
                    ar.checked = data.auto_repayment == 1;
                    ar.parentElement.classList.toggle('active', ar.checked);
                }
            } catch (error) {
        ...
        document.getElementById('planForm').addEventListener('submit', async function(e) {
            ...
            try {
                ...
                showToast('Network error or server error occurred. Please try again.', 'error');
            } finally {
