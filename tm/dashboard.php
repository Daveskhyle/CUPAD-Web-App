<?php
// tm/dashboard.php - Top Management Dashboard
session_start();
require_once '../includes/config.php';
$pdo = getDbConnection();

// Ensure User is Logged In and is a Top Manager (tm)
...
if (isset($_GET['action'])) {
    ...
    switch($action) {
        case 'load_activities':
            handleLoadActivities($pdo);
            break;
        case 'search_activities':
            handleSearchActivities($pdo);
            break;
...
function handleLoadActivities($pdo) {
    $page = isset($_GET['page']) ? (int)$_GET['page'] : 1;
    $limit = isset($_GET['limit']) ? (int)$_GET['limit'] : 10;
    $offset = ($page - 1) * $limit;
    $filter = $_GET['filter'] ?? 'all';

    // Top Management sees ALL activities globally
    $sql = "
...
function handleSearchActivities($pdo) {
    $query = $_GET['q'] ?? '';
    $searchTerm = "%$query%";
    $filter = $_GET['filter'] ?? 'all';
...
$stmt_ns->execute(['start_date' => $start_of_month, 'end_date' => $today]);
$global_monthly_savings = $stmt_ns->fetchColumn();

// 3. Global Monthly Disbursement
$sql_disb = "
    SELECT COALESCE(SUM(principal), 0) 
    FROM disbursements
    WHERE CAST(date AS DATE) BETWEEN :start_date AND :end_date";
$stmt_d = $pdo->prepare($sql_disb);
$stmt_d->execute(['start_date' => $start_of_month, 'end_date' => $today]);
$global_monthly_disbursed = $stmt_d->fetchColumn();

// 4. Global Active Loans Count & Outstanding Balance
$sql_active = "
    SELECT COUNT(*) as loan_count, COALESCE(SUM(remaining_balance), 0) as total_outstanding
    FROM disbursements
    WHERE remaining_balance > 0";
$stmt_a = $pdo->query($sql_active);
$active_data = $stmt_a->fetch();

$global_active_loans_count = $active_data['loan_count'] ?? 0;
$global_outstanding_balance = $active_data['total_outstanding'] ?? 0;

// 5. Total Global Active Clients
$sql_clients = "SELECT COUNT(*) FROM clients WHERE status = 'active'";
$global_total_clients = $pdo->query($sql_clients)->fetchColumn();

// 6. Zones Performance List
$sql_zone_perf = "
    SELECT 
        z.id, 
        z.name as zone_name,
        ...
        (SELECT COALESCE(SUM(CASE WHEN amount < 0 OR LOWER(s.type) IN ('withdrawal', 'return', 'adjust') THEN -ABS(amount) ELSE amount END), 0) 
         FROM saving_collections s JOIN clients c2 ON s.client_id = c2.id JOIN branches b2 ON c2.branch_id = b2.id JOIN areas a2 ON b2.area_id = a2.id
         WHERE a2.zone_id = z.id AND CAST(s.date AS DATE) BETWEEN :start_date1 AND :end_date1) as monthly_net_savings,
        (SELECT COALESCE(SUM(lc.amount_collected), 0) 
         FROM loan_collections lc JOIN clients c3 ON lc.client_id = c3.id JOIN branches b3 ON c3.branch_id = b3.id JOIN areas a3 ON b3.area_id = a3.id
         WHERE a3.zone_id = z.id AND CAST(lc.date AS DATE) BETWEEN :start_date2 AND :end_date2) as monthly_collections,
        (SELECT COALESCE(SUM(d.remaining_balance), 0) 
         FROM disbursements d JOIN clients c4 ON d.client_id = c4.id JOIN branches b4 ON c4.branch_id = b4.id JOIN areas a4 ON b4.area_id = a4.id
         WHERE a4.zone_id = z.id AND d.remaining_balance > 0) as outstanding_portfolio
    FROM zones z
    ORDER BY monthly_collections DESC, monthly_net_savings DESC
";
$stmt_zp = $pdo->prepare($sql_zone_perf);
$stmt_zp->execute([
...
$zone_performances = $stmt_zp->fetchAll();

// 7. Initial Activity Load (Global)
$initial_limit = 10;
$sql_init_act = "
    SELECT type, client_name, amount, date, officer FROM (
        SELECT 
...
<head>
    ...
    <title>Top Management Dashboard | CUPAD</title>
    ...
    <main class="max-w-7xl mx-auto px-4 py-6">
        ...
        <div class="dashboard-grid">
            <div class="dashboard-card card-gradient-blue">
                ...
                <div class="flex justify-between items-start relative z-10">
                    <div class="min-w-0">
                        ...
                        <div class="text-2xl lg:text-3xl font-extrabold mt-1 mb-2 truncate" title="₦<?php echo number_format($global_monthly_savings); ?>">₦<?php echo number_format($global_monthly_savings); ?></div>
            ...
            <div class="dashboard-card card-gradient-orange">
                ...
                <div class="flex justify-between items-start relative z-10">
                    <div class="min-w-0">
                        <div class="text-sm font-semibold uppercase opacity-90">Global Disbursed</div>
                        <div class="text-2xl lg:text-3xl font-extrabold mt-1 mb-2 truncate" title="₦<?php echo number_format($global_monthly_disbursed); ?>">₦<?php echo number_format($global_monthly_disbursed); ?></div>
            ...
            <div class="dashboard-card card-gradient-green">
                ...
                <div class="flex justify-between items-start relative z-10">
                    <div class="min-w-0">
                        ...
                        <div class="text-2xl lg:text-3xl font-extrabold mt-1 mb-2 truncate" title="₦<?php echo number_format($global_outstanding_balance); ?>">
                            ₦<?php echo number_format($global_outstanding_balance); ?>
                        </div>
                        <div class="text-xs bg-black/20 rounded-full px-3 py-1 inline-flex items-center">
                            <i class="fas fa-file-signature mr-1"></i> <?php echo number_format($global_active_loans_count); ?> Active Loans
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="dashboard-card card-gradient-purple">
                ...
                <div class="flex justify-between items-start relative z-10">
                    <div class="min-w-0">
                        ...
                        <div class="text-2xl lg:text-3xl font-extrabold mt-1 mb-2 truncate"><?php echo number_format($global_total_clients); ?></div>
                        <div class="text-xs bg-black/20 rounded-full px-3 py-1 inline-flex items-center truncate"><i class="fas fa-globe mr-1"></i> System-wide Active</div>
                    </div>
                </div>
            </div>
        </div>

        <!-- QUICK ACTIONS FOR TM -->
        <h2 class="text-lg font-bold text-gray-800 dark:text-white mb-3">Global Management</h2>
        <div class="quick-links-grid">
            <a href="zones.php" class="quick-link-card">
                <div class="quick-link-icon" style="background: linear-gradient(135deg, #8b5cf6, #6d28d9);"><i class="fas fa-layer-group"></i></div>
                <span class="quick-link-label">Zones</span>
            </a>
            <a href="areas.php" class="quick-link-card">
                <div class="quick-link-icon" style="background: linear-gradient(135deg, #ec4899, #be185d);"><i class="fas fa-map-marked-alt"></i></div>
                <span class="quick-link-label">Areas</span>
            </a>
            ...
            <a href="users.php" class="quick-link-card">
                <div class="quick-link-icon" style="background: linear-gradient(135deg, #f59e0b, #b45309);"><i class="fas fa-user-tie"></i></div>
                <span class="quick-link-label">Staff</span>
            </a>
            ...
            <a href="analytics.php" class="quick-link-card">
                <div class="quick-link-icon" style="background: linear-gradient(135deg, #0ea5e9, #0369a1);"><i class="fas fa-chart-bar"></i></div>
        ...
        <!-- ZONES PERFORMANCE (Global Level) -->
        <h2 class="text-lg font-bold text-gray-800 dark:text-white mb-3">Zones Overview</h2>
        <div class="co-scroll-container">
            <?php foreach ($zone_performances as $zone): ?>
            <div class="co-card">
                <div class="co-head">
                    <div class="co-name" title="<?php echo htmlspecialchars($zone['zone_name']); ?>">
                        <i class="fas fa-layer-group mr-1 text-purple-500"></i> <?php echo htmlspecialchars($zone['zone_name']); ?>
                    </div>
                    <div class="co-count"><i class="fas fa-users"></i> <?php echo number_format($zone['active_clients']); ?></div>
                </div>
                <div class="co-stat-row">
                    <span style="color:var(--text-secondary);">Monthly Net Savings</span>
                    <span style="color:var(--success-color); font-weight:700;">₦<?php echo number_format($zone['monthly_net_savings']); ?></span>
                </div>
                <div class="co-stat-row">
                    <span style="color:var(--text-secondary);">Monthly Collections</span>
                    <span style="color:var(--primary-color); font-weight:700;">₦<?php echo number_format($zone['monthly_collections']); ?></span>
                </div>
                <div class="co-stat-row" style="margin-top: 0.5rem; border-top: 1px dashed var(--border-color); padding-top: 0.5rem;">
                    ...
                    <span style="color:var(--warning-color); font-weight:700;">₦<?php echo number_format($zone['outstanding_portfolio']); ?></span>
                </div>
            </div>
            <?php endforeach; ?>
            <?php if(empty($zone_performances)): ?>
                <div class="text-gray-400 italic p-4">No zones configured yet.</div>
            <?php endif; ?>
        </div>

        <!-- GLOBAL WIDE SEARCH & FEED -->
        ...
        <div class="activities-container">
            <div class="flex justify-between mb-4">
                <h2 class="text-lg font-bold text-gray-800 dark:text-white">Global Activity Feed</h2>
            ...
            <div id="activities-list">
                ...
                <?php if(empty($initial_activities)): ?>
                    <div class="text-center py-8 text-gray-400">No system activities found.</div>
                <?php endif; ?>
            </div>
    ...
    <script>
        class EnhancedDashboard {
            ...
            showBrowserNotification(message) {
                if ("Notification" in window && Notification.permission === "granted") {
                    new Notification("CUPAD TM Dashboard", {
                        body: message,
