...
// tm/clients.php - Top Management Global Client View
session_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax']) && $_POST['ajax'] === 'update_guarantor') {
    ...
    try {
        // TM can update any client globally
...
if (isset($_GET['ajax']) && $_GET['ajax'] === 'get_clients_data') {
    ...
    $filter_zone = $_GET['zone'] ?? 'all';
    $filter_branch = $_GET['branch'] ?? 'all';
    $filter_status = $_GET['status'] ?? 'default';
    ...
    if ($filter_status === 'default') {
        // Active clients OR Inactive clients with an outstanding loan balance
        $where_conditions[] = "(c.status = 'active' OR (c.status != 'active' AND EXISTS (SELECT 1 FROM disbursements d_sub WHERE d_sub.client_id = c.id AND d_sub.remaining_balance > 0)))";
    } elseif ($filter_status === 'active') {
        $where_conditions[] = "c.status = 'active'";
    } elseif ($filter_status === 'inactive_owing') {
        $where_conditions[] = "c.status != 'active' AND EXISTS (SELECT 1 FROM disbursements d_sub WHERE d_sub.client_id = c.id AND d_sub.remaining_balance > 0)";
    ...
    if (!empty($search)) {
        $where_conditions[] = "(c.name LIKE :s1 OR c.`union` LIKE :s2 OR u.full_name LIKE :s3)";
        $params[':s1'] = "%$search%";
        $params[':s2'] = "%$search%";
        $params[':s3'] = "%$search%";
    }
    ...
    if ($filter_branch !== 'all') {
        $where_conditions[] = "c.branch_id = :branch";
        $params[':branch'] = $filter_branch;
    } elseif ($filter_zone !== 'all') {
        $where_conditions[] = "a.zone_id = :zone";
    ...
    try {
        // Function to bind params dynamically
        $bindParams = function($stmt) use ($params) {
        ...
        // Count Total
        $count_sql = "SELECT COUNT(c.id) FROM clients c LEFT JOIN users u ON c.officer_username = u.username LEFT JOIN branches b ON c.branch_id = b.id LEFT JOIN areas a ON b.area_id = a.id WHERE $where_sql";
        $stmt_count = $pdo->prepare($count_sql);
        $bindParams($stmt_count);
        $stmt_count->execute();
        $total_clients = $stmt_count->fetchColumn();

        // Total Savings
        $sav_sql = "SELECT SUM(sb.balance) FROM saving_balances sb JOIN clients c ON sb.client_id = c.id LEFT JOIN users u ON c.officer_username = u.username LEFT JOIN branches b ON c.branch_id = b.id LEFT JOIN areas a ON b.area_id = a.id WHERE $where_sql";
        $stmt_sav = $pdo->prepare($sav_sql);
        $bindParams($stmt_sav);
        $stmt_sav->execute();
        $total_savings = floatval($stmt_sav->fetchColumn() ?? 0);

        // Total Loans (all clients with outstanding balance, regardless of status)
        $loan_sql = "SELECT SUM(d.remaining_balance) FROM disbursements d WHERE d.remaining_balance > 0";
        if ($filter_branch !== 'all' || $filter_zone !== 'all' || !empty($search)) {
            $loan_sql = "SELECT SUM(d.remaining_balance) FROM disbursements d JOIN clients c ON d.client_id = c.id LEFT JOIN users u ON c.officer_username = u.username LEFT JOIN branches b ON c.branch_id = b.id LEFT JOIN areas a ON b.area_id = a.id WHERE $where_sql AND d.remaining_balance > 0";
        }
        $stmt_loan = $pdo->prepare($loan_sql);
        if ($filter_branch !== 'all' || $filter_zone !== 'all' || !empty($search)) {
            $bindParams($stmt_loan);
        }
        $stmt_loan->execute();
        ...
        foreach ($results as $row) {
            ...
            $row['zone_name'] = $row['zone_name'] ?? 'Global';
            $display_data[] = $row;
        }
...
if (isset($_GET['ajax']) && $_GET['ajax'] === 'get_client_history') {
    ...
    try {
        $stmt = $pdo->prepare("SELECT c.name, c.officer_username, u.full_name as officer_name, c.branch_id, b.name as branch_name, a.name as area_name, z.name as zone_name,
        ...
        echo json_encode([
            ...
            'zone_name' => $client['zone_name'] ?? 'Global',
...
<head>
    ...
    <title>Global Clients | CUPAD TM</title>
...
<body>
    ...
    <main class="max-w-7xl mx-auto px-4 py-6">
        <div class="mb-6">
            <h1 class="text-2xl font-extrabold text-gray-900 dark:text-white tracking-tight">Global Clients</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">System-wide overview of all registered clients.</p>
        </div>
        ...
        <div class="dashboard-grid">
            <div class="dashboard-card card-gradient-blue">
                ...
                <div class="flex justify-between items-start relative z-10">
                    <div class="min-w-0">
                        <div class="text-sm font-semibold uppercase opacity-90">Filtered Clients</div>
                        <div class="text-2xl lg:text-3xl font-extrabold mt-1 mb-2 truncate" id="totalClients">-</div>
                        <div class="text-xs bg-black/20 rounded-full px-3 py-1 inline-flex items-center"><i class="fas fa-globe mr-1"></i> Match Filter</div>
                    </div>
                </div>
            </div>
            ...
            <div class="dashboard-card card-gradient-red">
                ...
                <div class="flex justify-between items-start relative z-10">
                    <div class="min-w-0">
                        ...
                        <div class="text-xs bg-black/20 rounded-full px-3 py-1 inline-flex items-center"><i class="fas fa-file-invoice-dollar mr-1"></i> Remaining</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="flex flex-col md:flex-row gap-3 mb-6">
            <div class="search-container md:flex-1">
            ...
            <div class="flex flex-wrap gap-3 w-full md:w-auto">
                <select id="zoneSelect" class="custom-select flex-1 sm:flex-none sm:w-32" onchange="filterBranches(); resetAndLoad();">
                    <option value="all">All Zones</option>
                    <?php foreach ($zones as $z): ?>
                        <option value="<?php echo htmlspecialchars($z['id']); ?>">
                            <?php echo htmlspecialchars($z['name']); ?>
                        </option>
                    <?php endforeach; ?>
                </select>

                <select id="branchSelect" class="custom-select flex-1 sm:flex-none sm:w-36" onchange="resetAndLoad()">
                    <option value="all">All Branches</option>
                    <?php foreach ($all_branches as $br): ?>
                        <option value="<?php echo htmlspecialchars($br['id']); ?>" data-zone="<?php echo htmlspecialchars($br['zone_id']); ?>">
                            <?php echo htmlspecialchars($br['area_name'] . ' - ' . $br['name']); ?>
                        </option>
                    <?php endforeach; ?>
                </select>
                
                <select id="statusSelect" class="custom-select flex-1 sm:flex-none sm:w-40" onchange="resetAndLoad()">
                    <option value="default">Active + Owing</option>
                    <option value="active">Active Only</option>
                    <option value="inactive_owing">Inactive w/ Loan</option>
                    <option value="inactive">All Inactive</option>
                    <option value="all">All Clients</option>
                </select>

                <select id="sortSelect" class="custom-select flex-1 sm:flex-none sm:w-36" onchange="resetAndLoad()">
        ...
        <div class="modal-card">
            <div class="px-6 py-5 border-b border-gray-100 dark:border-slate-800 flex justify-between items-center">
                <div>
                    ...
                    <div class="text-xs text-gray-500 mt-0.5">Update client security details globally</div>
                </div>
    ...
    <script>
        ...
        function filterBranches() {
            ...
            Array.from(branchSelect.options).forEach(opt => {
                ...
                if (zoneId === 'all' || optZone === zoneId) {
                    opt.style.display = '';
                } else {
                    opt.style.display = 'none';
                }
            });
            branchSelect.value = 'all'; // Reset branch when zone changes
        }
        ...
        async function loadClients(append = false) {
            ...
            try {
                const res = await fetch(`?ajax=get_clients_data&page=${page}&search=${encodeURIComponent(search)}&sort=${sort}&branch=${branch}&zone=${zone}&status=${status}`);
                ...
                if(data.success) {
                    ...
                    if(data.clients.length === 0 && !append) {
                        ...
                        data.clients.forEach((c, index) => {
                            ...
                            if (c.status !== 'active') {
                                statusColor = '#ef4444'; // Red dot for inactive
                            } else if (c.outstanding_loan > 0) {
                                statusColor = (c.outstanding_loan > c.savings) ? '#ef4444' : '#f59e0b';
                            }
                            
                            let inactiveBadge = c.status !== 'active' ? `<span class="bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 text-[9px] px-1.5 py-0.5 rounded ml-2 uppercase font-bold shrink-0">Inactive</span>` : '';
                            ...
                            card.innerHTML = `
                                <div class="flex justify-between items-start">
                                    <div class="flex gap-3 items-center min-w-0 w-full pr-2">
                                        ...
                                        <div class="min-w-0 flex-1">
                                            <div class="flex items-center mb-0.5">
                                                <h3 class="font-bold text-gray-900 dark:text-white truncate" title="${c.name}">${c.name}</h3>
                                                ${inactiveBadge}
                                            </div>
                                            <span class="text-[10px] font-semibold bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 px-2 py-0.5 rounded-full uppercase inline-block truncate max-w-full">${c.union}</span>
                                            <div class="text-[10px] font-bold text-blue-600 dark:text-blue-400 mt-1 flex items-center gap-1">
                                                <i class="fas fa-user-tie shrink-0"></i> <span class="truncate">${c.officer_name}</span>
                                            </div>
                                            <div class="text-[10px] font-bold text-indigo-600 dark:text-indigo-400 mt-0.5 flex items-center gap-1">
                                                <i class="fas fa-map-marker-alt shrink-0"></i> <span class="truncate">${c.zone_name} &bull; ${c.branch_name}</span>
                                            </div>
                                        </div>
                                    </div>
                                    <button class="w-9 h-9 rounded-full bg-blue-50 dark:bg-slate-800 text-blue-500 dark:text-blue-400 flex items-center justify-center hover:bg-blue-100 shrink-0" onclick="viewHistory('${c.id}')"><i class="fas fa-history text-sm"></i></button>
                                </div>
                                ...
                                <div class="text-xs bg-gray-50 dark:bg-slate-800/50 p-3 rounded-xl border dark:border-slate-700 flex justify-between items-center">
                                    <span class="flex items-center gap-2 min-w-0 pr-2"><i class="fas fa-user-shield text-gray-400 shrink-0"></i> <span class="font-semibold truncate">${c.guarantor_name}</span></span>
                                    ${c.guarantor_phone !== 'N/A' ? `<a href="tel:${c.guarantor_phone}" class="font-mono text-blue-600 text-[11px] shrink-0">${c.guarantor_phone}</a>` : '<span class="text-gray-400 text-[11px] shrink-0">N/A</span>'}
                                </div>
                ...
                if(data.success) {
                    ...
                    $('historyModalSubtitle').innerHTML = `
                        <div class="flex gap-2 items-center flex-wrap">
                            <span class="text-[11px] font-bold bg-purple-50 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400 px-2 py-1 rounded border border-purple-100">
                                <i class="fas fa-globe mr-1"></i> ${data.zone_name}
                            </span>
        ...
        function renderHistory(filterType) {
            ...
            filtered.forEach((item, index) => {
                ...
                let txIdHtml = item.transaction_id ? `
                    <div class="text-[9px] font-mono text-gray-400 dark:text-gray-500 mt-1.5 flex items-center gap-1 cursor-pointer hover:text-blue-600 dark:hover:text-blue-400 transition-colors" onclick="copyTxId('${item.transaction_id}', this)" title="Copy Transaction ID">
                        <i class="far fa-copy"></i>
                        <span class="truncate max-w-[120px]">${item.transaction_id}</span>
                    </div>
                ` : '';
        ...
        function copyTxId(txId, el) {
            ...
            navigator.clipboard.writeText(txId).then(() => {
                ...
                el.innerHTML = `<i class="fas fa-check text-green-500"></i> <span class="truncate max-w-[120px]">Copied!</span>`;
