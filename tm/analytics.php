<?php
// tm/analytics.php - Top Management Analytics Dashboard (Optimized)
session_start();
require_once '../includes/config.php';

// Prevent Memory Exhaustion for Global Reporting
...
$tm_username = $_SESSION['username'] ?? '';
$user_id = $_SESSION['user_id'] ?? '';

// Fetch the TM's details
$stmt_t = $conn->prepare("SELECT full_name, profile_pic FROM users WHERE id = ?");
$stmt_t->execute([$user_id]);
$tm_info = $stmt_t->fetch(PDO::FETCH_ASSOC);

$full_name = !empty($tm_info['full_name']) ? $tm_info['full_name'] : 'Top Management';
$profile_pic = !empty($tm_info['profile_pic']) ? $tm_info['profile_pic'] : 'default_avatar.png';

// Fetch ALL active zones (Global View)
$zone_list =[];
$stmt_zones = $conn->query("SELECT id, name FROM zones ORDER BY name ASC");
while ($row = $stmt_zones->fetch(PDO::FETCH_ASSOC)) {
    $zone_list[] = $row;
}

// Fetch ALL active areas
$area_list =[];
$stmt_areas = $conn->query("SELECT id, name, zone_id FROM areas ORDER BY name ASC");
...
$branch_list =[];
$stmt_branches = $conn->query("SELECT id, name, area_id FROM branches WHERE status = 'active' ORDER BY name ASC");
while ($row = $stmt_branches->fetch(PDO::FETCH_ASSOC)) {
    $branch_list[] = $row;
}

// Fetch ALL active COs
$co_list =[];
$stmt_cos = $conn->query("SELECT id, username, full_name, branch_id FROM users WHERE role = 'co' AND status = 'active'");
while ($row = $stmt_cos->fetch(PDO::FETCH_ASSOC)) {
    $co_list[] = $row;
}

// --- HYBRID CACHE SYSTEM ---
function get_cache_key($date_from, $date_to, $search = '') {
    return md5('global_opt_' . $date_from . $date_to . $search . 'v_tm_analytics_v2');
}
...
if ($preset) {
    switch ($preset) {
        case 'today': $req_date_from = $req_date_to = date('Y-m-d'); break;
        case 'week': $req_date_from = date('Y-m-d', strtotime('monday this week')); $req_date_to = date('Y-m-d'); break;
        case 'month': $req_date_from = date('Y-m-01'); $req_date_to = date('Y-m-d'); break;
        case 'last_month':
            $req_date_from = date('Y-m-01', strtotime('last month'));
            $req_date_to = date('Y-m-t', strtotime('last month'));
            break;
    }
}

$dt_from = $req_date_from . ' 00:00:00';
$dt_to = $req_date_to . ' 23:59:59';

// --- DATA LOADING (OPTIMIZED) ---
$cache_dir = $base_path . 'cache/';
$cache_key = get_cache_key($req_date_from, $req_date_to);
$cached_data = get_cached_data($cache_key, $cache_dir); 

if (isset($_GET['force']) && $_GET['force'] == '1') $cached_data = null; // Force refresh

if ($cached_data) {
    $global_clients = $cached_data['global_clients'];
    $client_savings_balances = $cached_data['client_savings_balances'];
    $filtered_savings = $cached_data['filtered_savings'];
    $relevant_disbursements = $cached_data['relevant_disbursements'];
    $filtered_disbursements = $cached_data['filtered_disbursements'];
    $filtered_collections = $cached_data['filtered_collections'];
    $filtered_registrations = $cached_data['filtered_registrations'];
} else {

    // 1. All Clients
    $stmt = $conn->query("SELECT id, name, `union`, status, officer_username, branch_id FROM clients");
    $global_clients = $stmt->fetchAll(PDO::FETCH_ASSOC);
    $client_map = array_column($global_clients, 'name', 'id');
    
    // 2. All-Time Savings Balances per Client (Aggregated via SQL to save memory)
    $stmt_b = $conn->query("
        SELECT client_id, 
               SUM(CASE WHEN amount >= 0 AND LOWER(type) NOT IN ('withdrawal','return','adjust') THEN amount ELSE -ABS(amount) END) as bal
        FROM saving_collections 
        GROUP BY client_id
    ");
    $client_savings_balances = $stmt_b->fetchAll(PDO::FETCH_KEY_PAIR);

    // 3. Savings (ONLY for the requested date range)
    $stmt_s = $conn->prepare("SELECT client_id, amount, date, type, officer FROM saving_collections WHERE date >= ? AND date <= ?");
    $stmt_s->execute([$dt_from, $dt_to]);
    $filtered_savings = $stmt_s->fetchAll(PDO::FETCH_ASSOC);

    // 4. Relevant Disbursements (Active Loans OR Loans Disbursed/Paid Off in the period)
    $stmt_d = $conn->prepare("
        SELECT id, client_id, principal as principal_amount, total_payable, remaining_balance, date, payoff_date, officer 
        FROM disbursements 
        WHERE remaining_balance > 0 
           OR (date >= ? AND date <= ?) 
           OR (payoff_date >= ? AND payoff_date <= ?)
    ");
    $stmt_d->execute([$dt_from, $dt_to, $dt_from, $dt_to]);
    $relevant_disbursements = $stmt_d->fetchAll(PDO::FETCH_ASSOC);

    // Filter disbursements purely within range for totals
    $filtered_disbursements = array_filter($relevant_disbursements, function($d) use ($req_date_from, $req_date_to) {
        $d_date = substr($d['date'], 0, 10);
        return $d_date >= $req_date_from && $d_date <= $req_date_to;
    });

    // 5. Collections (ONLY for the requested date range)
    $stmt_c = $conn->prepare("SELECT client_id, amount_collected, date, officer FROM loan_collections WHERE date >= ? AND date <= ?");
    $stmt_c->execute([$dt_from, $dt_to]);
    $filtered_collections = $stmt_c->fetchAll(PDO::FETCH_ASSOC);

    // 6. Registrations (ONLY for the requested date range)
    $stmt_r = $conn->prepare("SELECT client_id, `union`, amount, date, officer FROM registrations WHERE date >= ? AND date <= ?");
    $stmt_r->execute([$dt_from, $dt_to]);
    ...
    save_to_cache([
        ...
        'client_savings_balances' => $client_savings_balances,
        'filtered_savings' => $filtered_savings,
        'relevant_disbursements' => $relevant_disbursements,
        'filtered_disbursements' => $filtered_disbursements,
        'filtered_collections' => $filtered_collections,
        'filtered_registrations' => $filtered_registrations
    ], $cache_key, $cache_dir);
}

// --- 5. METRICS CALCULATION (GLOBAL WIDE) ---
$total_clients = count(array_filter($global_clients, fn($c) => $c['status'] === 'active'));
...
foreach ($relevant_disbursements as $loan) {
...
if (isset($_GET['ajax']) && $_GET['ajax'] === '1') {
    ...
    // 6a. Main AJAX call - Global Dash & Zone Table
    if ($action === 'main') {
        $zone_stats =[];
        foreach ($zone_list as $zone) {
            $z_id = $zone['id'];
            $z_areas = array_filter($area_list, fn($a) => $a['zone_id'] == $z_id);
            $z_area_ids = array_column($z_areas, 'id');
            
            $z_branches = array_filter($branch_list, fn($b) => in_array($b['area_id'], $z_area_ids));
            $z_branch_ids = array_column($z_branches, 'id');
            
            $z_clients = array_filter($global_clients, fn($c) => in_array($c['branch_id'], $z_branch_ids));
            $z_client_ids = array_column($z_clients, 'id');
            
            $z_savings = array_filter($filtered_savings, fn($s) => in_array($s['client_id'], $z_client_ids));
            $z_disb = array_filter($filtered_disbursements, fn($d) => in_array($d['client_id'], $z_client_ids));
            $z_coll = array_filter($filtered_collections, fn($c) => in_array($c['client_id'], $z_client_ids));
            $z_regs = array_filter($filtered_registrations, fn($r) => in_array($r['client_id'], $z_client_ids));

            $z_dep = 0; $z_wit = 0;
            foreach ($z_savings as $s) {
                $amt = floatval($s['amount'] ?? 0); $type = strtolower($s['type'] ?? '');
                if ($amt < 0 || $type === 'withdrawal' || $type === 'return' || $type === 'adjust') $z_wit += abs($amt); else $z_dep += $amt;
            }

            $paid_off_count = 0; $overdue_count = 0;
            $all_zone_loans = array_filter($relevant_disbursements, fn($d) => in_array($d['client_id'], $z_client_ids));
            foreach ($z_clients as $client) {
                $c_loans = array_filter($all_zone_loans, fn($d) => $d['client_id'] === $client['id']);
            ...
            $z_cos = count(array_filter($co_list, fn($co) => in_array($co['branch_id'], $z_branch_ids)));

            $zone_stats[] = [
                'id' => $zone['id'], 
                'name' => $zone['name'], 
                'area_count' => count($z_areas),
                'branch_count' => count($z_branches),
                'co_count' => $z_cos,
                'clients' => count($z_clients), 
                'total_savings' => $z_dep, 
                'withdrawals' => abs($z_wit), 
                'net_savings' => $z_dep - $z_wit, 
                'disbursements' => array_sum(array_column($z_disb, 'principal_amount')), 
                'collections' => array_sum(array_column($z_coll, 'amount_collected')), 
                'registrations' => count($z_regs), 
        ...
        $daily_data =[];
        $period = new DatePeriod(new DateTime($req_date_from), new DateInterval('P1D'), (new DateTime($req_date_to))->modify('+1 day'));
        ...
        echo json_encode([
            ...
            'main_table' => $zone_stats,
            'dates' =>['from' => $req_date_from, 'to' => $req_date_to]
        ]);
        exit();
    }

    // 6b. Zone Details (Area Breakdown)
    if ($action === 'zone_details') {
        $z_id = $_GET['zone_id'] ?? '';
        $zone_areas = array_filter($area_list, fn($a) => $a['zone_id'] == $z_id);
        
        $area_stats =[];
        foreach ($zone_areas as $area) {
            $a_id = $area['id'];
            $a_branches = array_filter($branch_list, fn($b) => $b['area_id'] == $a_id);
            $a_branch_ids = array_column($a_branches, 'id');
            
            $a_clients = array_filter($global_clients, fn($c) => in_array($c['branch_id'], $a_branch_ids));
            ...
            $paid_off_count = 0; $overdue_count = 0;
            $all_area_loans = array_filter($relevant_disbursements, fn($d) => in_array($d['client_id'], $a_client_ids));
        ...
        echo json_encode(['areas' => $area_stats]);
        exit();
    }

    // 6c. Area Details (Branch Breakdown)
    if ($action === 'area_details') {
        ...
        foreach ($area_branches as $branch) {
            $b_id = $branch['id'];
            $b_clients = array_filter($global_clients, fn($c) => $c['branch_id'] == $b_id);
            ...
            $paid_off_count = 0; $overdue_count = 0;
            $all_branch_loans = array_filter($relevant_disbursements, fn($d) => in_array($d['client_id'], $b_client_ids));
    ...
    // 6d. Branch Details (CO Breakdown)
    if ($action === 'branch_details') {
        ...
        foreach ($branch_cos as $co) {
            $co_user = $co['username'];
            $officer_clients = array_filter($global_clients, fn($c) => $c['officer_username'] === $co_user);
            ...
            $paid_off_count = 0; $overdue_count = 0;
            $all_officer_loans = array_filter($relevant_disbursements, fn($d) => $d['officer'] === $co_user);
    ...
    // 6e. CO Details (Union Breakdown)
    if ($action === 'co_details') {
        $co_username = $_GET['co_username'] ?? '';
        $officer_clients = array_filter($global_clients, fn($c) => $c['officer_username'] === $co_username);
        $officer_disbursements_all = array_filter($relevant_disbursements, fn($d) => $d['officer'] === $co_username);
        
        $ajax_filtered_collections = array_filter($filtered_collections, fn($c) => $c['officer'] === $co_username && $c['date'] >= $aj_from && $c['date'] <= $aj_to." 23:59:59");
        $ajax_filtered_savings = array_filter($filtered_savings, fn($s) => $s['officer'] === $co_username && $s['date'] >= $aj_from && $s['date'] <= $aj_to." 23:59:59");
        $ajax_filtered_registrations = array_filter($filtered_registrations, fn($r) => $r['officer'] === $co_username && $r['date'] >= $aj_from && $r['date'] <= $aj_to." 23:59:59");
    ...
    // 6f. Union Details (Client Breakdown)
    if ($action === 'union_details') {
        $co_username = $_GET['co_username'] ?? '';
        $union_name = $_GET['union_name'] ?? '';
        $client_search = isset($_GET['q']) ? strtolower(trim($_GET['q'])) : '';

        $co_disbursements_all = array_filter($relevant_disbursements, fn($d) => $d['officer'] === $co_username);
        $ajax_filtered_collections = array_filter($filtered_collections, fn($c) => $c['officer'] === $co_username && $c['date'] >= $aj_from && $c['date'] <= $aj_to." 23:59:59");
        $ajax_filtered_savings = array_filter($filtered_savings, fn($s) => $s['officer'] === $co_username && $s['date'] >= $aj_from && $s['date'] <= $aj_to." 23:59:59");
        $ajax_filtered_disbursements = array_filter($filtered_disbursements, fn($d) => $d['officer'] === $co_username && $d['date'] >= $aj_from && $d['date'] <= $aj_to." 23:59:59");
        
        $target_clients = array_filter($global_clients, fn($c) => $c['officer_username'] === $co_username && $c['union'] === $union_name);
        ...
        foreach ($target_clients as $client) {
            ...
            // Fast lookup for all-time savings balance
            $savings_bal = $client_savings_balances[$cid] ?? 0;
...
<head>
    ...
    <title>Global Analytics | CUPAD</title>
...
<body>
    ...
    <header class="main-header">
        <nav class="navbar">
            <a href="dashboard.php" class="flex items-center gap-2 no-underline">
                ...
                <span class="text-xl font-extrabold text-blue-600 tracking-tight">CUPAD TM</span>
            </a>
    ...
    <main class="max-w-7xl mx-auto px-4 py-6">
        ...
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6">
            <div>
                <h1 class="text-2xl font-extrabold text-gray-900 dark:text-white">Global Analytics</h1>
                <div class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mt-1">
                    <span><i class="fas fa-globe text-blue-500"></i> Top Management, Global Head</span>
                    <span>&bull;</span>
        ...
        <div class="dashboard-grid">
            <div class="dashboard-card card-gradient-pink"> 
                ...
                <div class="flex justify-between items-start relative z-10"> 
                    <div>
                        <div class="text-sm font-semibold uppercase opacity-90">Global Active Clients</div> 
                        <div class="text-3xl font-extrabold mt-1 mb-2 count-up" id="metric_clients">0</div> 
                        <div class="text-xs bg-black/20 rounded-full px-3 py-1 inline-flex items-center"><i class="fas fa-globe mr-1"></i> System-wide Total</div> 
                    </div>
            ...
            <div class="dashboard-card card-gradient-blue"> 
                ...
                <div class="flex justify-between items-start relative z-10"> 
                    <div>
                        <div class="text-sm font-semibold uppercase opacity-90">Global Net Savings</div> 
            ...
            <div class="dashboard-card card-gradient-orange"> 
                ...
                <div class="flex justify-between items-start relative z-10"> 
                    <div>
                        <div class="text-sm font-semibold uppercase opacity-90">Global Disbursements</div> 
            ...
            <div class="dashboard-card card-gradient-green"> 
                ...
                <div class="flex justify-between items-start relative z-10"> 
                    <div>
                        <div class="text-sm font-semibold uppercase opacity-90">Global Collections</div> 
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
            <div class="bg-white dark:bg-gray-800 rounded-2xl p-5 border border-gray-200 dark:border-gray-700 shadow-sm"> 
                <h3 class="font-bold text-gray-800 dark:text-white mb-4 flex items-center gap-2"><i class="fas fa-heartbeat text-error"></i> Global Portfolio Health</h3> 
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl p-5 border border-gray-200 dark:border-gray-700 shadow-sm"> 
                <h3 class="font-bold text-gray-800 dark:text-white mb-4 flex items-center gap-2"><i class="fas fa-calculator text-primary"></i> Global Financials</h3> 
                <div class="space-y-3"> 
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl p-5 border border-gray-200 dark:border-gray-700 shadow-sm flex flex-col"> 
                <h3 class="font-bold text-gray-800 dark:text-white mb-2 flex items-center gap-2"><i class="fas fa-chart-pie text-secondary"></i> Global Portfolio Mix</h3> 
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 mb-8 border border-gray-200 dark:border-gray-700 shadow-sm">
            <h3 class="font-bold text-gray-800 dark:text-white mb-4 flex items-center gap-2"><i class="fas fa-chart-line text-primary"></i> Global Activity Trend</h3>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
            <div class="p-4 border-b border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row justify-between items-center gap-3">
                 <h3 class="font-bold text-lg text-gray-800 dark:text-white">Performance by Zone</h3>
                 <div class="relative w-full sm:w-64">
                    <i class="fas fa-search absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"></i>
                    <input type="text" id="mainSearchInput" placeholder="Search zones..." class="w-full pl-9 py-2 bg-gray-100 dark:bg-gray-900 border-none rounded-lg text-sm text-gray-700 dark:text-gray-300 focus:ring-2 focus:ring-primary">
                </div>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm whitespace-nowrap">
                    <thead class="bg-gray-50 dark:bg-gray-900/50 text-gray-500 dark:text-gray-400 uppercase text-xs font-semibold">
                        <tr>
                            <th class="p-4">Zone Name</th> <th class="p-4 text-center">Areas</th> <th class="p-4 text-center">Branches</th> <th class="p-4 text-center">COs</th> <th class="p-4 text-center">Clients</th> <th class="p-4 text-right">Deposits</th> <th class="p-4 text-right">Withdrawals</th> <th class="p-4 text-right">Net Savings</th> <th class="p-4 text-right">Disbursed</th> <th class="p-4 text-right">Collected</th> <th class="p-4 text-center">Reg.</th> <th class="p-4 text-center">Paid Off</th> <th class="p-4 text-center">Overdue</th>
                        </tr>
                    </thead>
                    <tbody id="mainTableBody" class="divide-y divide-gray-100 dark:divide-gray-700">
                        <tr><td colspan="13" class="p-8 text-center text-gray-500">Loading data...</td></tr>
                    </tbody>
    ...
    <script>
        ...
        // Modal State Machine for Deep Drill Down (TM Level)
        let modalState = {
            view: null, // 'areas', 'branches', 'cos', 'unions', 'clients'
            zone: null,
            area: null,
            branch: null,
            officer: null,
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            document.getElementById('modalSearchInput').addEventListener('input', debounce(() => { 
                if (modalState.view === 'areas') {
                    openZoneDetails(modalState.zone.id, modalState.zone.name);
                } else if (modalState.view === 'branches') {
        ...
        function renderTable(rows) {
            ...
            if(!rows || !rows.length) {
                tbody.innerHTML = '<tr><td colspan="13" class="p-8 text-center text-gray-500">No active zones found.</td></tr>';
            ...
            const tableHtml = rows.map(r => {
                ...
                return `<tr class="data-row hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer transition" data-name="${r.name.toLowerCase()}" onclick="openZoneDetails('${r.id}', '${r.name.replace(/'/g,"\\'")}')">
                    <td class="p-4 font-medium text-pink-600 dark:text-pink-400"><i class="fas fa-layer-group mr-1"></i> ${r.name}</td> 
                    <td class="p-4 text-center font-bold text-gray-700 dark:text-gray-300">${r.area_count}</td>
        ...
        async function openZoneDetails(zone_id, zone_name) {
            modalState.view = 'areas';
            modalState.zone = { id: zone_id, name: zone_name };
            
            document.getElementById('modalTitle').textContent = `Areas in ${zone_name}`;
            document.getElementById('modalSubtitle').textContent = `Performance by Area`;
            document.getElementById('modalBackButton').classList.add('hidden'); // Top level for TM modal
            document.getElementById('modalSearchInput').placeholder = 'Search areas...';
            document.getElementById('modalSearchInput').value = '';
            
            const thead = document.getElementById('modalTableHead');
            thead.innerHTML = `<tr> <th class="p-3 text-xs uppercase text-gray-500">Area Name</th> <th class="p-3 text-xs uppercase text-gray-500 text-center">Branches</th> <th class="p-3 text-xs uppercase text-gray-500 text-center">COs</th> <th class="p-3 text-xs uppercase text-gray-500 text-center">Clients</th> <th class="p-3 text-xs uppercase text-gray-500 text-right">Deposits</th> <th class="p-3 text-xs uppercase text-gray-500 text-right">Withdrawals</th> <th class="p-3 text-xs uppercase text-gray-500 text-right">Net Savings</th> <th class="p-3 text-xs uppercase text-gray-500 text-right">Disbursed</th> <th class="p-3 text-xs uppercase text-gray-500 text-right">Collections</th> <th class="p-3 text-xs uppercase text-gray-500 text-center">Overdue</th> </tr>`;
            
            document.getElementById('detailModal').classList.remove('hidden');
            const tbody = document.getElementById('modalTableBody');
            tbody.innerHTML = '<tr><td colspan="10" class="p-8 text-center"><i class="fas fa-spinner fa-spin text-2xl text-primary"></i></td></tr>';
            
            const from = document.getElementById('date_from').value;
            const to = document.getElementById('date_to').value;
            
            try {
                const res = await fetch(`?ajax=1&action=zone_details&zone_id=${zone_id}&date_from=${from}&date_to=${to}`);
                const data = await res.json();
                
                if(data.areas && data.areas.length) {
                    const search = document.getElementById('modalSearchInput').value.toLowerCase();
                    const filtered = data.areas.filter(a => a.name.toLowerCase().includes(search));
                    
                    if (filtered.length > 0) {
                        tbody.innerHTML = filtered.map(a => `
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer transition" onclick="openAreaDetails('${a.id}', '${a.name.replace(/'/g,"\\'")}')">
                               <td class="p-3 font-medium text-pink-600 dark:text-pink-400"><i class="fas fa-map-marked-alt mr-2"></i> ${a.name}</td>
                               <td class="p-3 text-center text-gray-500">${a.branch_count}</td>
                               <td class="p-3 text-center text-gray-500">${a.co_count}</td>
                               <td class="p-3 text-center text-gray-500">${a.clients}</td>
                               <td class="p-3 text-right font-mono text-success">₦${parseInt(a.total_savings).toLocaleString()}</td>
                               <td class="p-3 text-right font-mono text-error">₦${parseInt(a.withdrawals).toLocaleString()}</td>
                               <td class="p-3 text-right font-mono text-primary font-bold">₦${parseInt(a.net_savings).toLocaleString()}</td>
                               <td class="p-3 text-right font-mono text-warning">₦${parseInt(a.disbursements).toLocaleString()}</td>
                               <td class="p-3 text-right font-mono text-gray-600 dark:text-gray-300">₦${parseInt(a.collections).toLocaleString()}</td>
                               <td class="p-3 text-center">${a.overdue_count > 0 ? `<span class="px-2 py-0.5 rounded-full bg-red-100 text-red-600 font-bold">${a.overdue_count}</span>` : '-'}</td>
                            </tr>`).join('');
                    } else {
                        tbody.innerHTML = '<tr><td colspan="10" class="p-8 text-center text-gray-500">No matching areas.</td></tr>';
                    }
                } else {
                    tbody.innerHTML = '<tr><td colspan="10" class="p-8 text-center text-gray-500">No areas found in this zone.</td></tr>';
                }
            } catch(e) { console.error(e); tbody.innerHTML = '<tr><td colspan="10" class="p-8 text-center text-red-500">Error loading data.</td></tr>'; }
        }
        ...
        function closeModal() {
            ...
            modalState = { view: null, zone: null, area: null, branch: null, officer: null, union: null };
            document.getElementById('modalSearchInput').value = '';
        }
