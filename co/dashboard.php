<?php
// co/dashboard.php - Adapted for CUPADNAM_DB Schema
session_start();
...
if (isset($_GET['action'])) {
    ...
    switch($action) {
        case 'load_activities':
            handleLoadActivities($pdo, $username);
            break;
        case 'search_activities':
            handleSearchActivities($pdo, $username);
            break;
...
function handleSearchActivities($pdo, $username) {
    $query = $_GET['q'] ?? '';
    $searchTerm = "%$query%";
    ...
    $sql = "
        ...
        WHERE client_name LIKE :q OR type LIKE :q
        ORDER BY date DESC
        LIMIT 20
    ";
    ...
    <div class="activity-card" data-type="' . $activity_type_class . '" role="listitem">
        ...
        <div class="ac-content">
            ...
            <div class="ac-bottom">
                ...
                <div class="activity-date"><i class="far fa-clock"></i> ' . $formatted_date . '</div>
            </div>
        </div>
    </div>';
}
...
if ($user_data) {
    ...
    if (!empty($user_data['branch_name'])) $branch_name = $user_data['branch_name'];
...
// 2. Monthly Net Savings
$sql_net_savings = "SELECT COALESCE(SUM(CASE WHEN amount < 0 OR LOWER(type) IN ('withdrawal', 'return', 'adjust') THEN -ABS(amount) ELSE amount END), 0) FROM saving_collections WHERE officer = :uname AND DATE_FORMAT(date, '%Y-%m') = :m";
$stmt_ns = $pdo->prepare($sql_net_savings);
$stmt_ns->execute(['uname' => $username, 'm' => $current_month]);
$monthly_savings = $stmt_ns->fetchColumn();

// 3. Monthly Disbursement
$sql_disb = "SELECT SUM(principal) FROM disbursements WHERE officer = :uname AND DATE_FORMAT(date, '%Y-%m') = :m";
$stmt_d = $pdo->prepare($sql_disb);
$stmt_d->execute(['uname' => $username, 'm' => $current_month]);
$monthly_disbursed = $stmt_d->fetchColumn() ?: 0;

// 4. Active Loans Count
$sql_active = "SELECT COUNT(DISTINCT client_id) FROM disbursements WHERE officer = :uname AND status != 'completed' AND remaining_balance > 0";
$stmt_a = $pdo->prepare($sql_active);
$stmt_a->execute(['uname' => $username]);
$active_loans_count = $stmt_a->fetchColumn() ?: 0;

// 5. Portfolio & Union Performance (Exactly matches client_financial_summary.php calculations)
$sql_union = "
    SELECT 
        c.id, 
        c.`union`,
...
while ($row = $stmt_un->fetch(PDO::FETCH_ASSOC)) {
    $savings = floatval($row['total_savings'] ?? 0);
    $loans = floatval($row['loan_balance'] ?? 0);
    
    $grand_savings += $savings;
    $grand_loans += $loans;
    
    // Group dynamically using PHP to eliminate case/spacing duplication!
    $u_name = trim($row['union'] ?? '');
    if ($u_name === '') {
        $u_name = 'Unassigned';
    } else {
        $u_name = ucwords(strtolower($u_name)); // e.g., "union a" -> "Union A"
    }
    
    if (!isset($union_stats_raw[$u_name])) {
        $union_stats_raw[$u_name] =['name' => $u_name, 'clients' => 0, 'savings' => 0, 'loans' => 0];
    }
    
    $union_stats_raw[$u_name]['clients']++;
    $union_stats_raw[$u_name]['savings'] += $savings;
    $union_stats_raw[$u_name]['loans'] += $loans;
}

$grand_net = $grand_savings - $grand_loans;

// Sort grouped unions alphabetically
ksort($union_stats_raw);
$union_stats = array_values($union_stats_raw);

// 6. Initial Activity Load (Limit 7)
$sql_init_act = "
    SELECT type, client_name, amount, date FROM (
        SELECT 'Saving' as type, c.name as client_name, s.amount, s.date 
        FROM saving_collections s JOIN clients c ON s.client_id = c.id 
...
$stmt_ia->execute([$username, $username, $username, $username]);
$initial_activities = $stmt_ia->fetchAll();
$has_more = count($initial_activities) >= 7;
...
<head>
    ...
    <style>
        ...
        .union-scroll-container { display: grid; grid-template-columns: repeat(auto-fill, minmax(260px, 1fr)); gap: 1.25rem; margin-bottom: 2rem; }
        .union-card { background: var(--bg-card); border-radius: var(--border-radius-lg); padding: 1.25rem; border: 1px solid var(--border-color); box-shadow: var(--shadow-sm); position: relative; }
        .union-head { display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.75rem; border-bottom: 1px solid var(--border-color); padding-bottom: 0.75rem; }
        .union-name { font-weight: 700; font-size: 1rem; color: var(--text-primary); }
        .union-count { font-size: 0.75rem; background: var(--bg-primary); padding: 3px 10px; border-radius: 12px; color: var(--text-secondary); }
        .union-stat-row { display: flex; justify-content: space-between; font-size: 0.85rem; margin-top: 0.5rem; }
        ...
        .filter-tabs { display: flex; gap: 0.5rem; overflow-x: auto; padding-bottom: 5px; scrollbar-width: none; }
        ...
        @media (max-width: 768px) {
            ...
            .union-scroll-container { display: flex; overflow-x: auto; scroll-snap-type: x mandatory; gap: 1rem; padding-bottom: 0.5rem; margin-bottom: 1.5rem; scrollbar-width: none; }
            .union-scroll-container::-webkit-scrollbar { display: none; }
            .union-card { min-width: 280px; scroll-snap-align: start; }
...
<body>
    ...
    <main class="max-w-7xl mx-auto px-4 py-6">
        ...
        <div class="mb-6 text-center">
            <h1 class="text-2xl font-extrabold text-gray-900 dark:text-white"><?php echo $greeting . ', ' . explode(' ', trim($full_name))[0]; ?></h1>
            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                <i class="fas fa-map-marker-alt mr-1"></i>
                <?php echo htmlspecialchars($zone_name . ' → ' . $area_name . ' → ' . $branch_name); ?> 
        ...
        <div class="dashboard-grid">
            <div class="dashboard-card card-gradient-blue">
                ...
                <div class="flex justify-between items-start relative z-10">
                    <div class="min-w-0">
                        ...
                        <div class="text-2xl lg:text-3xl font-extrabold mt-1 mb-2 truncate" title="₦<?php echo number_format($monthly_savings); ?>">₦<?php echo number_format($monthly_savings); ?></div>
            ...
            <a href="disbursement.php" class="dashboard-card card-gradient-orange" style="text-decoration: none; color: inherit;">
                <i class="fas fa-hand-holding-usd card-bg-icon"></i>
                <div class="flex justify-between items-start relative z-10">
                    <div class="min-w-0">
                        <div class="text-sm font-semibold uppercase opacity-90">Monthly Disbursed</div>
                        <div class="text-2xl lg:text-3xl font-extrabold mt-1 mb-2 truncate" title="₦<?php echo number_format($monthly_disbursed); ?>">₦<?php echo number_format($monthly_disbursed); ?></div>
            ...
            <div class="dashboard-card card-gradient-green">
                ...
                <div class="flex justify-between items-start relative z-10">
                    <div class="min-w-0">
                        ...
                        <div class="text-2xl lg:text-3xl font-extrabold mt-1 mb-2 truncate"><?php echo number_format($active_loans_count); ?></div>
                        <div class="text-xs bg-black/20 rounded-full px-3 py-1 inline-flex items-center"><i class="fas fa-users mr-1"></i> Clients</div>
                    </div>
                </div>
            </div>
            ...
            <div class="dashboard-card card-gradient-purple">
                <i class="fas fa-map-marker-alt card-bg-icon"></i>
                <div class="flex justify-between items-start relative z-10">
                    <div class="min-w-0">
                        <div class="text-sm font-semibold uppercase opacity-90">Location</div>
                        <div class="text-2xl lg:text-3xl font-extrabold mt-1 mb-2 truncate" title="<?php echo htmlspecialchars($branch_name); ?>"><?php echo htmlspecialchars($branch_name); ?></div>
                        <div class="text-xs bg-black/20 rounded-full px-3 py-1 inline-flex items-center truncate"><i class="fas fa-user mr-1"></i> <?php echo htmlspecialchars($username); ?></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <h2 class="text-lg font-bold text-gray-800 dark:text-white mb-3">Quick Actions</h2>
        <div class="quick-links-grid">
            <a href="combined_collection.php" class="quick-link-card">
                <div class="quick-link-icon" style="background: linear-gradient(135deg, #10b981, #059669);"><i class="fas fa-coins"></i></div>
                <span class="quick-link-label">Collect</span>
            </a>
            <a href="saving_collection.php" class="quick-link-card">
                <div class="quick-link-icon" style="background: linear-gradient(135deg, #3b82f6, #1d4ed8);"><i class="fas fa-piggy-bank"></i></div>
                <span class="quick-link-label">Savings</span>
            </a>
            <a href="loan_collection.php" class="quick-link-card">
                <div class="quick-link-icon" style="background: linear-gradient(135deg, #8b5cf6, #7c3aed);"><i class="fas fa-money-bill-wave"></i></div>
                <span class="quick-link-label">Repay</span>
            </a>
            <a href="disbursement.php" class="quick-link-card">
                <div class="quick-link-icon" style="background: linear-gradient(135deg, #3b82f6, #2563eb);"><i class="fas fa-hand-holding-usd"></i></div>
                <span class="quick-link-label">Disburse</span>
            </a>
            <a href="saving_withdrawal.php" class="quick-link-card">
                <div class="quick-link-icon" style="background: linear-gradient(135deg, #ef4444, #dc2626);"><i class="fas fa-wallet"></i></div>
                <span class="quick-link-label">Withdraw</span>
            </a>
            <a href="clients.php" class="quick-link-card">
                <div class="quick-link-icon" style="background: linear-gradient(135deg, #10b981, #059669);"><i class="fas fa-users"></i></div>
                <span class="quick-link-label">Clients</span>
            </a>
            <a href="registration.php" class="quick-link-card">
                <div class="quick-link-icon" style="background: linear-gradient(135deg, #6366f1, #4f46e5);"><i class="fas fa-user-plus"></i></div>
                <span class="quick-link-label">Register</span>
            </a>
            <a href="union_groups.php" class="quick-link-card">
                <div class="quick-link-icon" style="background: linear-gradient(135deg, #0ea5e9, #0284c7);"><i class="fas fa-sitemap"></i></div>
                <span class="quick-link-label">Unions</span>
            </a>
            ...
            <a href="client_financial_summary.php" class="quick-link-card">
                <div class="quick-link-icon" style="background: linear-gradient(135deg, #64748b, #475569);"><i class="fas fa-file-invoice"></i></div>
                <span class="quick-link-label">Summary</span>
            </a>
            <a href="close_client.php" class="quick-link-card">
                <div class="quick-link-icon" style="background: linear-gradient(135deg, #f43f5e, #be123c);"><i class="fas fa-user-times"></i></div>
                <span class="quick-link-label">Close Acc</span>
            </a>
            <a href="../passkey_setup.php" class="quick-link-card">
                <div class="quick-link-icon" style="background: linear-gradient(135deg, #3b82f6, #1d4ed8);"><i class="fas fa-fingerprint"></i></div>
                <span class="quick-link-label">Passkey</span>
            </a>
        </div>

        <!-- PORTFOLIO OVERVIEW -->
        <h2 class="text-lg font-bold text-gray-800 dark:text-white mb-3">Portfolio Overview</h2>
        <div class="bg-white dark:bg-[#1e293b] rounded-xl p-4 mb-6 border border-gray-200 dark:border-gray-700 shadow-sm flex justify-between items-center text-center">
            <div class="flex-1">
                <div class="text-[0.65rem] font-bold text-gray-500 uppercase mb-1">Total Savings</div>
                <div class="text-lg md:text-xl font-black text-green-600">₦<?php echo number_format($grand_savings); ?></div>
            </div>
            <div class="flex-1 border-l border-r border-gray-100 dark:border-gray-700">
                <div class="text-[0.65rem] font-bold text-gray-500 uppercase mb-1">Total Debt</div>
                <div class="text-lg md:text-xl font-black text-red-600">₦<?php echo number_format($grand_loans); ?></div>
            </div>
            <div class="flex-1">
                <div class="text-[0.65rem] font-bold text-gray-500 uppercase mb-1">Net Position</div>
                <div class="text-lg md:text-xl font-black <?php echo $grand_net >= 0 ? 'text-blue-600' : 'text-orange-500'; ?>">
                    ₦<?php echo number_format($grand_net); ?>
                </div>
            </div>
        </div>

        <!-- UNION PERFORMANCE (Safely Grouped) -->
        <div class="flex justify-between items-center mb-3">
            <h2 class="text-lg font-bold text-gray-800 dark:text-white">Union Breakdown</h2>
            <span class="text-xs font-semibold text-gray-500 bg-gray-200 dark:bg-gray-700 px-2 py-1 rounded-lg">
                <?php echo count($union_stats); ?> Active Unions
            </span>
        </div>
        
        <div class="union-scroll-container">
            <?php foreach ($union_stats as $u): ?>
            <div class="union-card">
                <div class="union-head">
                    <div class="union-name"><?php echo htmlspecialchars($u['name']); ?></div>
                    <div class="union-count"><i class="fas fa-users"></i> <?php echo $u['clients']; ?></div>
                </div>
                <div class="union-stat-row">
                    <span style="color:var(--text-secondary);">Savings</span>
                    <span style="color:var(--success-color); font-weight:700;">₦<?php echo number_format($u['savings']); ?></span>
                </div>
                <div class="union-stat-row">
                    <span style="color:var(--text-secondary);">Loans</span>
                    <span style="color:var(--error-color); font-weight:700;">₦<?php echo number_format($u['loans']); ?></span>
                </div>
            </div>
            <?php endforeach; ?>
            <?php if(empty($union_stats)): ?>
                <div class="text-gray-400 italic p-4">No union data available.</div>
            <?php endif; ?>
        </div>
        ...
        <div class="search-filter-container">
            <div class="search-row">
                <div style="position:relative; flex:1;">
                    ...
                    <input type="text" id="activity-search" class="search-input" placeholder="Search client name...">
                </div>
        ...
        <div class="activities-container">
            <div class="flex justify-between mb-4">
                <h2 class="text-lg font-bold text-gray-800 dark:text-white">Activity Feed</h2>
    ...
    <script>
        class EnhancedDashboard {
            ...
            init() {
                ...
                document.querySelectorAll('.filter-tab').forEach(tab => {
                    tab.addEventListener('click', (e) => {
                        ...
                        this.filterLocal(e.target.dataset.filter);
                    });
                });

                document.getElementById('clear-filters').addEventListener('click', () => {
                    document.getElementById('activity-search').value = '';
                    this.filterLocal('all');
                });
            ...
            showBrowserNotification(message) {
                if ("Notification" in window && Notification.permission === "granted") {
                    new Notification("CUPAD Dashboard", {
                        body: message,
            ...
            async performSearch(query) {
                ...
                const url = window.location.href.split('?')[0];
                try {
                    const res = await fetch(`${url}?action=search_activities&q=${encodeURIComponent(query)}`);
                    const data = await res.json();
                    if (data.success) {
                        document.getElementById('activities-list').innerHTML = data.html;
            ...
            filterLocal(type) {
                const items = document.querySelectorAll('.activity-card');
                items.forEach(item => {
            ...
            async loadMoreActivities() {
                ...
                try {
                    const res = await fetch(`${url}?action=load_activities&page=${this.currentPage}&limit=${this.limit}`);
                    if (!res.ok) throw new Error('Network error');
                    const data = await res.json();
                    if (data.success) {
                        document.getElementById('activities-list').insertAdjacentHTML('beforeend', data.html);
                        this.hasMore = data.has_more;
                        if (!this.hasMore) document.getElementById('load-more-btn-container').style.display = 'none';
                    }
                } catch (error) {
                    this.currentPage--; 
                } finally {
