...
// --- DATABASE CONNECTION (Matched to dashboard.php) ---
if (file_exists($base_path . 'includes/config.php')) {
    require_once $base_path . 'includes/config.php';
}
if (file_exists($base_path . 'db.php')) {
...
// --- SECURITY CHECK ---
if (empty($username) || ($user_role !== 'co' && $user_role !== 'admin' && $user_role !== 'bm')) {
    if (isset($_SERVER['HTTP_X_REQUESTED_WITH'])) {
        header('Content-Type: application/json');
...
// --- FETCH USER & LOCATION DETAILS FROM SQL (Matched to dashboard.php) ---
$stmt = $pdo->prepare("
    SELECT u.*, 
...
if (!$user_data) {
    die("Error: User profile record not found in database.");
}

// Values from SQL
$full_name = $user_data['full_name'] ?? $_SESSION['name'] ?? 'Credit Officer';
$email = $user_data['email'] ?? '';
$role = $user_data['role'] ?? $user_role;
$branch_name = $user_data['branch_name'] ?? 'My Branch';
$area_name = $user_data['area_name'] ?? 'Unassigned';
$zone_name = $user_data['zone_name'] ?? 'Unassigned';

// Parse Failed Login Attempts Count from JSON column
$failed_attempts = json_decode($user_data['failed_login_attempts'] ?? '[]', true);
$failed_count = is_array($failed_attempts) ? count($failed_attempts) : 0;

// Resolve Profile Picture
$profile_pic_filename = !empty($user_data['profile_pic']) ? $user_data['profile_pic'] : 'default_avatar.png';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['form_action']) && $_POST['form_action'] === 'update_profile') {
        ...
        if (!filter_var($new_email, FILTER_VALIDATE_EMAIL)) $field_errors['email'] = 'Invalid email address.';

        if (empty($field_errors)) {
            $new_filename = $user_data['profile_pic'] ?? 'default_avatar.png';
            ...
            if (isset($_FILES['settings_profile_picture']) && $_FILES['settings_profile_picture']['error'] === UPLOAD_ERR_OK) {
                ...
                if ($image_info !== false && in_array($ext, $allowed_exts) && $file['size'] <= 5 * 1024 * 1024) {
                    ...
                    $relative_path = 'uploads/' . $username . '_' . time() . '.' . $ext;
                    $target_path = $base_path . $relative_path;

                    if (move_uploaded_file($file['tmp_name'], $target_path)) {
                        // Unlink old custom picture
                        if (!empty($new_filename) && strpos($new_filename, 'default') === false && file_exists($base_path . $new_filename)) {
                            @unlink($base_path . $new_filename);
                        }
                        $new_filename = $relative_path;
                        $profile_pic_path = $base_path . $new_filename;
                        $has_valid_pic = true;
                    } else {
                        $field_errors['settings_profile_picture'] = 'Failed to move uploaded file.';
                    }
                } else {
                    $field_errors['settings_profile_picture'] = 'Invalid image or file size exceeds 5MB.';
                }
            }

            if (empty($field_errors)) {
                // Update strictly using full_name column in MySQL DB
                ...
                if ($executed) {
                    $_SESSION['name'] = $new_full_name; // Update Active Session Name
                    ...
                    $response_data['message'] = 'Database error: Unable to update user profile.';
                }
            } else {
    ...
    if (isset($_POST['form_action']) && $_POST['form_action'] === 'change_password') {
        ...
        // Check password (supports hashed and plaintext legacy passwords)
        if (!password_verify($current, $stored) && $current !== $stored) {
            $field_errors['currentPassword'] = 'Incorrect password.';
        }
        if (strlen($new) < 6) {
            $field_errors['newPassword'] = 'Password too short (min 6 characters).';
        }
        if ($new !== $confirm) {
        ...
        if (empty($field_errors)) {
            ...
            $pass_stmt = $pdo->prepare("UPDATE users SET password = :password WHERE username = :username");
            
            if ($pass_stmt->execute([':password' => $new_hash, ':username' => $username])) {
                $response_data['success'] = true;
                $response_data['message'] = 'Password changed successfully.';
            } else {
                $response_data['message'] = 'Database error: Failed to update password.';
            }
        } else {
            $response_data['field_errors'] = $field_errors;
        }
...
<body class="bg-gray-50 dark:bg-slate-900">
    ...
    <header class="main-header">
        <nav class="navbar">
            ...
            <div class="flex items-center gap-3">
                ...
                <!-- Header Avatar -->
                <div id="headerAvatarContainer" class="w-9 h-9 rounded-full overflow-hidden border-2 border-blue-500 bg-gray-100 dark:bg-slate-700 flex items-center justify-center">
                    <?php if($has_valid_pic): ?>
                        <img id="headerAvatar" src="<?php echo htmlspecialchars($profile_pic_path); ?>" alt="Profile" class="w-full h-full object-cover">
                        <div id="headerAvatarFallback" class="w-full h-full flex items-center justify-center hidden">
                            <i class="fas fa-user text-gray-400 dark:text-gray-300"></i>
                        </div>
                    <?php else: ?>
                        <div id="headerAvatarFallback" class="w-full h-full flex items-center justify-center">
                            <i class="fas fa-user text-gray-400 dark:text-gray-300"></i>
                        </div>
                        <img id="headerAvatar" src="" alt="Profile" class="w-full h-full object-cover hidden">
                    <?php endif; ?>
                </div>
    ...
    <main class="max-w-3xl mx-auto px-4 py-6">
            ...
            <div class="relative z-10 text-center">
                ...
                <div class="avatar-wrapper">
                    <?php if($has_valid_pic): ?>
                        ...
                        <div id="heroAvatarFallback" class="avatar-fallback hidden"><i class="fas fa-user"></i></div>
                    <?php else: ?>
                    ...
                    <label for="settings_profile_picture" class="avatar-edit-btn" title="Upload Photo">
                ...
                <div class="inline-flex gap-2 flex-wrap justify-center">
                    <!-- Location Hierarchy matching dashboard.php -->
                    <span class="px-3 py-1 bg-white/20 rounded-full text-xs backdrop-blur-sm">
                        <i class="fas fa-map-marker-alt mr-1"></i> <?php echo htmlspecialchars($zone_name . ' → ' . $area_name . ' → ' . $branch_name); ?>
                    </span>
                    <?php if($role): ?>
                    <span class="px-3 py-1 bg-white/20 rounded-full text-xs backdrop-blur-sm uppercase">
                        <i class="fas fa-user-shield mr-1"></i> <?php echo htmlspecialchars($role); ?>
                    </span>
                    <?php endif; ?>
                </div>
            </div>
        ...
        <div class="stat-grid">
            <div class="stat-card">
                <div class="stat-val text-green-500"><?php echo ucfirst($user_data['status'] ?? 'active'); ?></div>
        ...
        <div class="card">
            ...
            <div class="p-5">
                <form id="profileForm" enctype="multipart/form-data">
                    ...
                    <input type="file" id="settings_profile_picture" name="settings_profile_picture" class="hidden" accept="image/png, image/jpeg, image/jpg, image/gif, image/webp">
        ...
        <div class="card p-5">
            ...
            <div class="timeline">
                ...
                <?php endif; ?>
                <div class="timeline-item">
                    <div class="text-sm font-bold text-gray-800 dark:text-white">Profile Synchronized</div>
                    <div class="text-xs text-gray-500">Connected to Database</div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        const heroFallback = $('heroAvatarFallback');
                ...
                try {
                    ...
                    if(data.success) {
                        ...
                        if(formId === 'profileForm' && data.data) {
                            ...
                            if(data.data.hasValidPic && data.data.profilePicPath) {
                                const newPicUrl = data.data.profilePicPath + '?t=' + new Date().getTime();

                                const heroImg = $('heroAvatar');
                                const heroFb = $('heroAvatarFallback');
                                heroImg.src = newPicUrl;
                                heroImg.classList.remove('hidden');
                                if(heroFb) heroFb.classList.add('hidden');

                                const headerImg = $('headerAvatar');
                                const headerFb = $('headerAvatarFallback');
                                headerImg.src = newPicUrl;
                                headerImg.classList.remove('hidden');
                                if(headerFb) headerFb.classList.add('hidden');
                            }
                        }
