    ...
    if (in_array($type, $blocked_types)) {
        ...
        try {
            ...
            elseif ($type === 'return' || $type === 'withdrawal') {
                ...
                if ($active_loan) {
                    $new_rem_bal = max(0, floatval($active_loan['remaining_balance']) - $amount_deducted);
                    $loan_paid_off = ($new_rem_bal <= 0.01);
                    if ($loan_paid_off) {
                        $new_rem_bal = 0;
                    }
                    $status = $loan_paid_off ? 'completed' : 'active';
                    $payoff_sql = $loan_paid_off ? ", payoff_date = ?" : "";
                    ...
                    $upd_loan = $pdo->prepare("
                        ...
                        WHERE id = ?
                    ");
                    $loan_params = [$new_rem_bal, $status];
                    if ($loan_paid_off) {
                        $loan_params[] = $input_date;
                    }
                    $loan_params[] = $active_loan['id'];
                    $upd_loan->execute($loan_params);
            ...
            $note = $loan_paid_off
                ? "Loan fully paid via WTH withdrawal"
                : ($pic_path ? "Image: " . basename($pic_path) : "Used for loan deduction");
            $stmt->execute([
