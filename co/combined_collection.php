...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_action']) && $_POST['ajax_action'] === 'save_client') {
    ...
    try {
        ...
        if ($withdrawal_type !== '' && ($withdrawal_amt > 0 || $withdrawal_type === 'return')) {
            ...
            // Match saving_withdrawal: analytics counts WTH payoffs via this exact note
            if ($loan_paid_off) {
                $note = 'Loan fully paid via WTH withdrawal';
            } elseif ($uploaded_pic_path) {
                $note = 'Image: ' . basename($uploaded_pic_path);
            } else {
                $note = ($withdrawal_type === 'withdrawal' || $withdrawal_type === 'return')
                    ? 'Used for loan deduction'
                    : null;
            }
