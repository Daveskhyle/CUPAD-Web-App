<?php
// bm/dashboard.php - Branch Manager Dashboard (Corrected Activity Feed)
session_start();
...
if (isset($_GET['action'])) {
    ...
    switch($action) {
        case 'load_activities':
            handleLoadActivities($pdo, $branch_id);
            break;
        case 'search_activities':
            handleSearchActivities($pdo, $branch_id);
            break;
...
function handleLoadActivities($pdo, $branch_id) {
    $page = isset($_GET['page']) ? (int)$_GET['page'] : 1;
    $limit = isset($_GET['limit']) ? (int)$_GET['limit'] : 10;
    $offset = ($page - 1) * $limit;
    $filter = $_GET['filter'] ?? 'all';

    // Get all CO usernames in this branch
    $stmt_cos = $pdo->prepare("SELECT username FROM users WHERE branch_id = :bid AND role = 'co' AND status = 'active'");
    $stmt_cos->execute(['bid' => $branch_id]);
    ...
    // MODIFIED: Added activity_id from each table for stable sorting
    $sql = "
    ...
    // MODIFIED: Added activity_id to ORDER BY as a tie-breaker
    $sql .= "
...
function handleSearchActivities($pdo, $branch_id) {
    $query = $_GET['q'] ?? '';
    $searchTerm = "%$query%";
    $filter = $_GET['filter'] ?? 'all';
...
function getActivityColor($type) {
    $colors =[
        ...
        'Disbursement' => '#3b82f6', // Blue
        'Registration' => '#6366f1', // Indigo
        'default' => '#6b7280'
    ];
...
// --- STATS CALCULATION (Branch Level) ---
$current_month = date('Y-m');

// 2. Branch Monthly Net Savings
$sql_net_savings = "
    SELECT COALESCE(SUM(CASE WHEN amount < 0 OR LOWER(s.type) IN ('withdrawal', 'return', 'adjust') THEN -ABS(amount) ELSE amount END), 0) 
    FROM saving_collections s 
    JOIN clients c ON s.client_id = c.id
    WHERE c.branch_id = :bid AND DATE_FORMAT(s.date, '%Y-%m') = :m";
$stmt_ns = $pdo->prepare($sql_net_savings);
$stmt_ns->execute(['bid' => $branch_id, 'm' => $current_month]);
$branch_monthly_savings = $stmt_ns->fetchColumn();

// 3. Branch Monthly Disbursement
$sql_disb = "
    SELECT COALESCE(SUM(principal), 0) 
    FROM disbursements d
    JOIN clients c ON d.client_id = c.id
    WHERE c.branch_id = :bid AND DATE_FORMAT(d.date, '%Y-%m') = :m";
$stmt_d = $pdo->prepare($sql_disb);
$stmt_d->execute(['bid' => $branch_id, 'm' => $current_month]);
$branch_monthly_disbursed = $stmt_d->fetchColumn();

// 4. Branch Active Loans Count & Outstanding Balance
$sql_active = "
    SELECT COUNT(*) as loan_count, COALESCE(SUM(d.remaining_balance), 0) as total_outstanding
    FROM disbursements d
    JOIN clients c ON d.client_id = c.id
    WHERE c.branch_id = :bid AND d.remaining_balance > 0";
$stmt_a = $pdo->prepare($sql_active);
$stmt_a->execute(['bid' => $branch_id]);
$active_data = $stmt_a->fetch();

$branch_active_loans_count = $active_data['loan_count'] ?? 0;
$branch_outstanding_balance = $active_data['total_outstanding'] ?? 0;

// 5. Total Branch Active Clients
$sql_clients = "
    SELECT COUNT(*) FROM clients 
    WHERE branch_id = :bid AND status = 'active'";
$stmt_c = $pdo->prepare($sql_clients);
$stmt_c->execute(['bid' => $branch_id]);
$branch_total_clients = $stmt_c->fetchColumn();

// 6. Credit Officers Performance
$sql_co_perf = "
    SELECT 
        u.username, 
        u.full_name,
        (SELECT COUNT(*) FROM clients c WHERE c.officer_username = u.username AND c.branch_id = :bid1 AND c.status = 'active') as active_clients,
        (SELECT COALESCE(SUM(CASE WHEN amount < 0 OR LOWER(s.type) IN ('withdrawal', 'return', 'adjust') THEN -ABS(amount) ELSE amount END), 0) 
         FROM saving_collections s JOIN clients c2 ON s.client_id = c2.id 
         WHERE s.officer = u.username AND c2.branch_id = :bid2 AND DATE_FORMAT(s.date, '%Y-%m') = :m1) as monthly_net_savings,
        (SELECT COALESCE(SUM(lc.amount_collected), 0) 
         FROM loan_collections lc JOIN clients c3 ON lc.client_id = c3.id 
         WHERE lc.officer = u.username AND c3.branch_id = :bid3 AND DATE_FORMAT(lc.date, '%Y-%m') = :m2) as monthly_collections,
        (SELECT COALESCE(SUM(d.remaining_balance), 0) 
         FROM disbursements d JOIN clients c4 ON d.client_id = c4.id 
         WHERE d.officer = u.username AND c4.branch_id = :bid4 AND d.remaining_balance > 0) as outstanding_portfolio
    FROM users u
    WHERE u.branch_id = :bid5 AND u.role = 'co' AND u.status = 'active'
    ORDER BY monthly_collections DESC, monthly_net_savings DESC
";
$stmt_co = $pdo->prepare($sql_co_perf);
$stmt_co->execute([
    'bid1' => $branch_id, 
    'bid2' => $branch_id, 'm1' => $current_month, 
    'bid3' => $branch_id, 'm2' => $current_month, 
    'bid4' => $branch_id, 
    'bid5' => $branch_id
]);
$co_performances = $stmt_co->fetchAll();
...
<head>
    ...
    <title>Branch Manager Dashboard | CUPAD</title>
    ...
    <style>
        ...
        .co-scroll-container { display: grid; grid-template-columns: repeat(auto-fill, minmax(260px, 1fr)); gap: 1.25rem; margin-bottom: 2rem; }
    ...
    <main class="max-w-7xl mx-auto px-4 py-6">
        ...
        <div class="mb-6 text-center">
            ...
            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                ...
                Branch Manager, <?php echo htmlspecialchars($branch_name); ?> 
                <span class="mx-2">•</span>
                <i class="fas fa-map-marker-alt mr-1"></i>
                <?php echo htmlspecialchars($zone_name . ' → ' . $area_name); ?>
            </p>
        </div>
            ...
            <div class="dashboard-card card-gradient-blue">
                ...
                <div class="flex justify-between items-start relative z-10">
                    <div class="min-w-0">
                        ...
                        <div class="text-2xl lg:text-3xl font-extrabold mt-1 mb-2 truncate" title="₦<?php echo number_format($branch_monthly_savings); ?>">₦<?php echo number_format($branch_monthly_savings); ?></div>
            ...
            <a href="disbursement.php" class="dashboard-card card-gradient-orange" style="text-decoration: none; color: inherit;">
                ...
                <div class="flex justify-between items-start relative z-10">
                    <div class="min-w-0">
                        ...
                        <div class="text-2xl lg:text-3xl font-extrabold mt-1 mb-2 truncate" title="₦<?php echo number_format($branch_monthly_disbursed); ?>">₦<?php echo number_format($branch_monthly_disbursed); ?></div>
            ...
            <div class="dashboard-card card-gradient-green">
                ...
                <div class="flex justify-between items-start relative z-10">
                    <div class="min-w-0">
                        ...
                        <div class="text-2xl lg:text-3xl font-extrabold mt-1 mb-2 truncate" title="₦<?php echo number_format($branch_outstanding_balance); ?>">
                            ₦<?php echo number_format($branch_outstanding_balance); ?>
                        </div>
                        <div class="text-xs bg-black/20 rounded-full px-3 py-1 inline-flex items-center">
                            <i class="fas fa-file-signature mr-1"></i> <?php echo number_format($branch_active_loans_count); ?> Active Loans
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="dashboard-card card-gradient-purple">
                ...
                <div class="flex justify-between items-start relative z-10">
                    <div class="min-w-0">
                        ...
                        <div class="text-2xl lg:text-3xl font-extrabold mt-1 mb-2 truncate"><?php echo number_format($branch_total_clients); ?></div>
                        <div class="text-xs bg-black/20 rounded-full px-3 py-1 inline-flex items-center truncate"><i class="fas fa-users mr-1"></i> Active</div>
                    </div>
                </div>
            </div>
        </div>

        <!-- QUICK ACTIONS FOR BM -->
        <h2 class="text-lg font-bold text-gray-800 dark:text-white mb-3">Branch Management</h2>
        <div class="quick-links-grid">
            ...
            <a href="loan.php" class="quick-link-card">
                <div class="quick-link-icon" style="background: linear-gradient(135deg, #3b82f6, #2563eb);"><i class="fas fa-file-invoice-dollar"></i></div>
                <span class="quick-link-label">Loans</span>
            </a>
        ...
        <h2 class="text-lg font-bold text-gray-800 dark:text-white mb-3">Credit Officers Overview</h2>
        <div class="co-scroll-container">
            <?php foreach ($co_performances as $co): ?>
            <div class="co-card">
                <div class="co-head">
                    <div class="co-name" title="<?php echo htmlspecialchars($co['username']); ?>"><?php echo htmlspecialchars(!empty($co['full_name']) ? explode(' ', trim($co['full_name']))[0] : $co['username']); ?></div>
                    <div class="co-count"><i class="fas fa-users"></i> <?php echo number_format($co['active_clients']); ?></div>
                </div>
                <div class="co-stat-row">
                    <span style="color:var(--text-secondary);">Monthly Net Savings</span>
                    <span style="color:var(--success-color); font-weight:700;">₦<?php echo number_format($co['monthly_net_savings']); ?></span>
                </div>
                <div class="co-stat-row">
                    <span style="color:var(--text-secondary);">Monthly Collections</span>
                    <span style="color:var(--primary-color); font-weight:700;">₦<?php echo number_format($co['monthly_collections']); ?></span>
                </div>
                <div class="co-stat-row" style="margin-top: 0.5rem; border-top: 1px dashed var(--border-color); padding-top: 0.5rem;">
                    <span style="color:var(--text-secondary);">Outstanding Loans</span>
                    <span style="color:var(--warning-color); font-weight:700;">₦<?php echo number_format($co['outstanding_portfolio']); ?></span>
                </div>
            </div>
            <?php endforeach; ?>
            <?php if(empty($co_performances)): ?>
                <div class="text-gray-400 italic p-4">No active Credit Officers assigned to this branch.</div>
            <?php endif; ?>
        </div>

        <!-- BRANCH WIDE SEARCH & FEED -->
        ...
        <div class="activities-container">
            <div class="flex justify-between mb-4">
                <h2 class="text-lg font-bold text-gray-800 dark:text-white">Branch Activity Feed</h2>
            ...
            <div id="activities-list">
                ...
                <?php if(empty($initial_activities)): ?>
                    <div class="text-center py-8 text-gray-400">No branch activities found.</div>
                <?php endif; ?>
            </div>
    ...
    <script>
        class EnhancedDashboard {
            ...
            showBrowserNotification(message) {
                if ("Notification" in window && Notification.permission === "granted") {
                    new Notification("CUPAD BM Dashboard", {
                        body: message,
