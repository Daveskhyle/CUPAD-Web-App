...
// bm/clients.php - Branch Manager Client View (PDO Version - UX Improved)
session_start();
...
// Fetch BM's Branch ID
 ...
 $full_name = $user_data['full_name'] ?? 'Branch Manager';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax']) && $_POST['ajax'] === 'update_guarantor') {
    ...
    try {
        $stmt = $pdo->prepare("UPDATE clients SET guarantor_name = ?, guarantor_phone = ? WHERE id = ? AND branch_id = ?");
        if ($stmt->execute([$g_name, $g_phone, $client_id, $branch_id])) {
            if($stmt->rowCount() > 0) {
                ...
                echo json_encode(['success' => false, 'message' => 'Client not found in your branch.']);
            }
        } else {
...
if (isset($_GET['ajax']) && $_GET['ajax'] === 'get_clients_data') {
    ...
    try {
        $stmt = $pdo->prepare("SELECT COUNT(id) FROM clients WHERE branch_id = ? AND status = 'active' AND (name LIKE ? OR `union` LIKE ?)");
        $stmt->execute([$branch_id, $search_term, $search_term]);
        $total_clients = $stmt->fetchColumn();

        $stmt = $pdo->prepare("SELECT SUM(sb.balance) 
                                FROM saving_balances sb 
                                JOIN clients c ON sb.client_id = c.id
                                WHERE c.branch_id = ? AND c.status = 'active' AND (c.name LIKE ? OR c.`union` LIKE ?)");
        $stmt->execute([$branch_id, $search_term, $search_term]);
        $total_savings = floatval($stmt->fetchColumn() ?? 0);

        $stmt = $pdo->prepare("SELECT SUM(d.remaining_balance) 
                                FROM disbursements d 
                                JOIN clients c ON d.client_id = c.id
                                WHERE c.branch_id = ? AND d.remaining_balance > 0");
        ...
        $sql = "SELECT 
                ...
                WHERE c.branch_id = :bid 
                  AND c.status = 'active' 
                  AND (c.name LIKE :search OR c.`union` LIKE :search2 OR u.full_name LIKE :search3)
                ORDER BY $order_by
                LIMIT :limit OFFSET :offset";
        
        $stmt = $pdo->prepare($sql);
        $stmt->bindValue(':bid', $branch_id);
...
if (isset($_GET['ajax']) && $_GET['ajax'] === 'get_client_history') {
    ...
    try {
        $stmt = $pdo->prepare("SELECT c.name, c.officer_username, u.full_name as officer_name, c.branch_id,
...
<head>
    ...
    <title>Branch Clients | CUPAD BM</title>
...
<body>
    ...
    <main class="max-w-7xl mx-auto px-4 py-6">
        <div class="mb-6">
            <h1 class="text-2xl font-extrabold text-gray-900 dark:text-white tracking-tight">Branch Clients</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Overview of all clients under your supervision.</p>
        </div>
            ...
            <div class="dashboard-card card-gradient-blue">
                ...
                <div class="flex justify-between items-start relative z-10">
                    <div class="min-w-0">
                        ...
                        <div class="text-xs bg-black/20 rounded-full px-3 py-1 inline-flex items-center"><i class="fas fa-users mr-1"></i> Active</div>
                    </div>
                </div>
            </div>
            ...
            <div class="dashboard-card card-gradient-red">
                ...
                <div class="flex justify-between items-start relative z-10">
                    <div class="min-w-0">
                        ...
                        <div class="text-xs bg-black/20 rounded-full px-3 py-1 inline-flex items-center"><i class="fas fa-file-invoice-dollar mr-1"></i> Total Remaining</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="flex flex-col md:flex-row gap-3 mb-6">
            ...
            <div class="flex gap-3 w-full md:w-auto">
                <select id="sortSelect" class="custom-select flex-1 md:w-48" onchange="resetAndLoad()">
    ...
    <script>
        ...
        async function loadClients(append = false) {
            ...
            try {
                const res = await fetch(`?ajax=get_clients_data&page=${page}&search=${encodeURIComponent(search)}&sort=${sort}`);
                    ...
                    if(data.clients.length === 0 && !append) {
                        ...
                        data.clients.forEach((c, index) => {
                            ...
                            card.innerHTML = `
                                ...
                                <div class="flex gap-2 bg-gray-50 dark:bg-slate-800/50 rounded-xl p-3 border dark:border-slate-700">
