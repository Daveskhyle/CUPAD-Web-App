...
// bm/saving_collection.php - Branch Manager Savings Collection
session_start();
...
function record_savings_sql($client_id, $amount, $date, $officer, $branch_id) {
    try {
        global $pdo;
        
        // Security: Ensure client belongs to BM's branch
        ...
        if ($stmt_check->fetchColumn() != $branch_id) {
            return['success' => false, 'message' => 'Unauthorized client branch mismatch.'];
        }
...
function get_savings_transactions_sql($branch_id, $co_filter = 'all', $union_filter = 'all', $date_filter = 'current_month', $page = 1, $limit = 10) {
    try {
        global $pdo;
        $sql = "SELECT sc.transaction_id, c.name as client_name, sc.amount as amount_collected, sc.date, sc.officer 
                ...
                WHERE c.branch_id = ? AND sc.type = 'deposit'";
        
        $params = [$branch_id];

        if ($co_filter !== 'all') {
            $sql .= " AND c.officer_username = ?";
            $params[] = $co_filter;
        }
...
function get_savings_stats_sql($branch_id) {
    try {
        global $pdo;

        $stmt = $pdo->prepare("SELECT COUNT(DISTINCT c.id) FROM clients c WHERE c.branch_id = ? AND c.status = 'active' AND EXISTS (SELECT 1 FROM savings s WHERE s.client_id = c.id AND s.balance > 0 AND s.status = 'active')");
        $stmt->execute([$branch_id]);
        $active_savers = $stmt->fetchColumn();

        $stmt = $pdo->prepare("SELECT COALESCE(SUM(s.balance), 0) FROM savings s JOIN clients c ON s.client_id = c.id WHERE c.branch_id = ? AND s.status = 'active'");
        $stmt->execute([$branch_id]);
        $total_savings = $stmt->fetchColumn();

        $stmt = $pdo->prepare("SELECT COALESCE(SUM(sc.amount), 0) FROM saving_collections sc JOIN clients c ON sc.client_id = c.id WHERE c.branch_id = ? AND sc.type = 'deposit' AND YEAR(sc.date) = YEAR(CURDATE()) AND MONTH(sc.date) = MONTH(CURDATE())");
        $stmt->execute([$branch_id]);
...
if (isset($_GET['ajax_action']) && $_GET['ajax_action'] === 'get_transactions') {
    ...
    $result = get_savings_transactions_sql($branch_id, $co_filter, $union_filter, $date_filter, $page, $limit);
    $page_total = 0;
...
// Fetch clients directly by branch
$clients_data =[];
$clients_by_id =[];
try {
...
$savings_by_client =[];
try {
    $client_ids = array_column($clients_data, 'id');
    if (!empty($client_ids)) {
...
$stats = get_savings_stats_sql($branch_id);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif (!$allow_weekend_collection && is_weekend($date)) $response['message'] = 'Savings collection not allowed on weekends.';
        ...
        // Record saving using BM's username as the processor
        $result = record_savings_sql($client_id, $amount, $date, $username, $branch_id);
        if ($result['success']) {
...
$js_clients_data = json_encode($clients_data);
...
<head>
    ...
    <style>
        ...
        .input-field { background: transparent; border: none; width: 100%; color: var(--text-primary); outline: none; font-size: 0.95rem; appearance: none; -webkit-appearance: none; } .input-field option { background-color: var(--bg-secondary); color: var(--text-primary); padding: 10px; }
...
<body>
    ...
    <main class="max-w-7xl mx-auto px-4 py-6">
        <div class="mb-6">
            <h1 class="text-2xl font-extrabold text-gray-900 dark:text-white">Savings Collection</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400">Record branch deposits and view history</p>
        </div>

        <div class="dashboard-grid">
            <div class="dashboard-card card-gradient-green"><i class="fas fa-users card-bg-icon"></i><div class="flex justify-between items-start relative z-10"><div><div class="text-sm font-semibold uppercase opacity-90">Branch Savers</div><div class="text-3xl font-extrabold mt-1 mb-2 count-up" data-target="<?php echo $stats['active_savers']; ?>">0</div><div class="text-xs bg-black/20 rounded-full px-3 py-1 inline-flex items-center"><i class="fas fa-hashtag mr-1"></i> Active</div></div><div class="w-12 h-12 bg-white/20 backdrop-blur rounded-xl flex items-center justify-center text-xl"><i class="fas fa-user-plus"></i></div></div></div>
            <div class="dashboard-card card-gradient-emerald"><i class="fas fa-piggy-bank card-bg-icon"></i><div class="flex justify-between items-start relative z-10"><div><div class="text-sm font-semibold uppercase opacity-90">Total Savings</div><div class="text-3xl font-extrabold mt-1 mb-2">₦<span class="count-up" data-target="<?php echo $stats['total_savings']; ?>" data-currency="true">0</span></div><div class="text-xs bg-black/20 rounded-full px-3 py-1 inline-flex items-center"><i class="fas fa-chart-line mr-1"></i> All Time</div></div><div class="w-12 h-12 bg-white/20 backdrop-blur rounded-xl flex items-center justify-center text-xl"><i class="fas fa-coins"></i></div></div></div>
            <div class="dashboard-card card-gradient-blue"><i class="fas fa-calendar-check card-bg-icon"></i><div class="flex justify-between items-start relative z-10"><div><div class="text-sm font-semibold uppercase opacity-90">Collected Month</div><div class="text-3xl font-extrabold mt-1 mb-2">₦<span class="count-up" data-target="<?php echo $stats['month_total']; ?>" data-currency="true">0</span></div><div class="text-xs bg-black/20 rounded-full px-3 py-1 inline-flex items-center"><i class="fas fa-calendar-day mr-1"></i> Performance</div></div><div class="w-12 h-12 bg-white/20 backdrop-blur rounded-xl flex items-center justify-center text-xl"><i class="fas fa-wallet"></i></div></div></div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div class="lg:col-span-1">
                <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-700 sticky top-24">
                    <div class="p-5 border-b border-gray-100 dark:border-slate-700">
                        <h2 class="text-lg font-bold flex items-center gap-2 text-gray-800 dark:text-white"><i class="fas fa-plus-circle text-green-500"></i> Record Savings</h2>
                    </div>
                    ...
                    <form id="savingsForm" class="p-5 space-y-5">
                        ...
                        <div id="client-info-card" class="hidden rounded-xl border border-gray-100 dark:border-slate-700 overflow-hidden">
                            <div class="px-3 py-2 text-xs font-bold text-white bg-gray-500 flex justify-between"><span>Financial Snapshot</span><span class="bg-black/20 px-2 rounded-full">Live</span></div>
                            <div class="p-3 bg-gray-50 dark:bg-slate-900/50 grid grid-cols-2 gap-2 text-sm">
                                <div><span class="text-[10px] text-gray-400 uppercase">Savings Balance</span><div id="info-savings" class="font-bold text-green-600">₦0</div></div>
                                <div class="text-right"><span class="text-[10px] text-gray-400 uppercase">Loan Balance</span><div id="info-loan" class="font-bold text-red-600">₦0</div></div>
                            </div>
                            <div class="px-3 pb-3 bg-gray-50 dark:bg-slate-900/50">
                                <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 mb-1"><div id="info-progress" class="bg-green-500 h-1.5 rounded-full transition-all duration-500" style="width: 0%"></div></div>
                                <div class="flex justify-between text-[10px]"><span class="text-gray-400">Loan Coverage</span><span class="font-bold text-green-600" id="info-coverage">0%</span></div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="input-group flex items-center">
                                ...
                                <input type="number" id="amount" name="amount" min="<?php echo $min_savings_amount; ?>" max="<?php echo $max_savings_amount; ?>" step="0.01" required class="input-field font-bold p-0 pr-0" placeholder="<?php echo number_format($min_savings_amount); ?> - <?php echo number_format($max_savings_amount); ?>">
                            </div>
                            <div class="text-xs text-gray-400 mt-1">Min: ₦<?php echo number_format($min_savings_amount); ?> | Max: ₦<?php echo number_format($max_savings_amount); ?></div>
                        </div>

                        <div>
                            <div class="flex justify-between items-center mb-1"><label class="text-xs font-bold text-gray-500 uppercase block">Date</label><button type="button" id="btn-set-now" class="text-[10px] font-bold text-green-600 hover:underline">SET NOW</button></div>
                            <div class="input-group py-2"><input type="datetime-local" id="date" name="date" value="<?php echo date('Y-m-d\TH:i'); ?>" class="input-field text-sm p-0 pr-0" <?php echo $savings_date_readonly ? 'readonly' : ''; ?>></div>
                            <?php if (!$allow_weekend_collection): ?><div class="text-xs text-orange-500 mt-1"><i class="fas fa-exclamation-triangle"></i> Weekend collection disabled</div><?php endif; ?>
                        </div>
            ...
            <div class="lg:col-span-2">
                <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-700 min-h-[500px] flex flex-col">
                    <div class="p-5 border-b border-gray-100 dark:border-slate-700 flex flex-wrap gap-4 justify-between items-center">
                        <h2 class="text-lg font-bold text-gray-800 dark:text-white">Recent Transactions</h2>
                        <div class="flex flex-wrap items-center gap-2">
                            <select id="history-co-filter" class="text-xs bg-gray-100 dark:bg-slate-700 border-none rounded-lg px-2 py-1 outline-none cursor-pointer font-semibold text-blue-600">
                            ...
                            <div class="flex border border-gray-200 dark:border-slate-600 rounded-lg overflow-hidden ml-1">
                                ...
                                <button class="filter-date-btn active text-xs px-3 py-1 bg-green-100 text-green-600 font-bold" data-val="current_month">This Month</button>
                    ...
                    <div class="p-4 border-t border-gray-100 dark:border-slate-700 text-center flex justify-between items-center">
                        <span class="text-xs text-gray-500" id="pagination-info">...</span>
                        <div class="flex gap-2">
                            <button id="prev-page" class="w-8 h-8 flex items-center justify-center rounded-lg border border-gray-200 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-slate-700 text-xs"><i class="fas fa-chevron-left"></i></button>
                            <button id="next-page" class="w-8 h-8 flex items-center justify-center rounded-lg border border-gray-200 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-slate-700 text-xs"><i class="fas fa-chevron-right"></i></button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        let state = { currentUnion: null, coFilter: 'all', unionFilter: 'all', dateFilter: 'current_month', currentPage: 1, limit: 10, isLoading: false };
        ...
        const els = { 
            ...
            infoCoverage: document.getElementById('info-coverage'), 
            amountInput: document.getElementById('amount'), 
            historyContainer: document.getElementById('history-container'), 
            form: document.getElementById('savingsForm'), 
        ...
        function initUnions() { 
            ...
            unions.forEach((u, i) => { 
                ...
                btn.type = 'button'; 
                btn.className = `px-3 py-1 rounded-full text-xs font-bold border transition ${i === 0 ? 'bg-green-600 text-white border-green-600' : 'bg-white text-gray-500 border-gray-200 hover:bg-gray-50 dark:bg-slate-700 dark:text-gray-300 dark:border-slate-600'}`; 
                btn.textContent = u; 
        ...
        function selectUnion(u, btn) { 
            state.currentUnion = u; 
            ...
            if(btn) btn.className = 'px-3 py-1 rounded-full text-xs font-bold border transition bg-green-600 text-white border-green-600'; 
            populateClients(u, els.coFilter.value); 
        }

        function populateClients(u, co) { 
            els.clientSelect.innerHTML = '<option value="">Select Client...</option>'; 
            if(u) {
                const filteredClients = getClients(u, co);
                filteredClients.forEach(c => { els.clientSelect.innerHTML += `<option value="${c.id}">${c.name}</option>`; }); 
            }
            hideInfo(); 
        }
        ...
        async function loadClientFinancialData(cid) { 
            try { 
                const r = await fetch(`?ajax_action=get_client_financial_data&client_id=${cid}`); 
                const d = await r.json(); 
                if (d.success) { 
                    els.infoSavings.textContent = formatCurrency(d.savings_balance); 
                    els.infoLoan.textContent = formatCurrency(d.loan_balance); 
                    savingsByClient[cid] = d.savings_balance; 
                    const c = d.loan_balance > 0 ? Math.min(100, (d.savings_balance / d.loan_balance) * 100) : 100; 
                    els.infoProgress.style.width = c + '%'; 
                    els.infoCoverage.textContent = Math.round(c) + '%'; 
                } 
        ...
        els.clientSelect.addEventListener('change', async (e) => { 
            ...
            if (!cid) { hideInfo(); return; } 
            els.clientInfoCard.classList.remove('hidden'); 
            els.infoSavings.textContent = savingsByClient[cid] ? formatCurrency(savingsByClient[cid]) : '...'; 
            await loadClientFinancialData(cid); 
        });
        ...
        async function loadTransactions(page = 1) { 
            ...
            try { 
                const r = await fetch(`?ajax_action=get_transactions&page=${page}&limit=${state.limit}&co=${state.coFilter}&union=${state.unionFilter}&date_filter=${state.dateFilter}`); 
                const d = await r.json(); 
                if (d.success) { 
                    renderFeed(d.data); 
        ...
        function renderFeed(rows) { 
            ...
            rows.forEach(i => { 
                els.historyContainer.insertAdjacentHTML('beforeend', `<div class="activity-card"><div class="ac-icon-box" style="background: ${GREEN_COLOR}15; color: ${GREEN_COLOR};"><i class="fas fa-piggy-bank"></i></div><div class="ac-content"><div class="ac-top"><div class="client-name">${i.client_name}</div><div class="activity-amount" style="color: ${GREEN_COLOR};">₦${parseInt(i.amount_collected).toLocaleString()}</div></div><div class="ac-bottom"><span style="font-size:0.65rem; font-weight:700; background:${GREEN_COLOR}15; color:${GREEN_COLOR}; padding:2px 6px; rounded:4px;">DEPOSIT</span><div class="activity-date"><i class="fas fa-user-tie"></i> ${i.officer} &bull; <i class="far fa-clock"></i> ${i.date.substring(5,16)}</div></div></div></div>`); 
            }); 
        }

        function updatePagination(m) { 
            state.currentPage = m.current_page; 
            const s = m.total_records > 0 ? ((m.current_page-1)*m.limit)+1 : 0; 
            els.paginationInfo.textContent = `${s}-${Math.min(m.total_records, m.current_page*m.limit)} of ${m.total_records}`; 
            els.prevPage.disabled = !m.has_prev; 
            els.nextPage.disabled = !m.has_next; 
            els.prevPage.onclick = () => loadTransactions(m.current_page - 1); 
            els.nextPage.onclick = () => loadTransactions(m.current_page + 1); 
        }

        els.form.addEventListener('submit', async (e) => { 
            e.preventDefault(); 
            const amt = parseFloat(els.amountInput.value); 
            const dv = document.getElementById('date').value; 
            
            if (amt < minSavingsAmount) { showToast(`Amount must be at least ₦${minSavingsAmount.toLocaleString()}`, 'error'); return; } 
            if (amt > maxSavingsAmount) { showToast(`Amount cannot exceed ₦${maxSavingsAmount.toLocaleString()}`, 'error'); return; } 
            if (!allowWeekendCollection && dv) { 
                const sd = new Date(dv); 
                if (sd.getDay() === 0 || sd.getDay() === 6) { showToast('Savings collection not allowed on weekends', 'error'); return; } 
            } 
            ...
            try { 
                ...
                if (rs.success) { 
                    ...
                    const am = parseFloat(fd.get('amount')); 
                    
                    document.querySelector('.card-gradient-emerald .count-up').dataset.target = parseFloat(document.querySelector('.card-gradient-emerald .count-up').dataset.target) + am; 
                    if (!savingsByClient[cid] || savingsByClient[cid] === 0) document.querySelector('.card-gradient-green .count-up').dataset.target = parseFloat(document.querySelector('.card-gradient-green .count-up').dataset.target) + 1; 
                    document.querySelector('.card-gradient-blue .count-up').dataset.target = parseFloat(document.querySelector('.card-gradient-blue .count-up').dataset.target) + am; 
                    
                    animateNumbers(); 
                    
                    if (cid) { 
                        savingsByClient[cid] = (savingsByClient[cid] || 0) + am; 
                        if (els.clientSelect.value === cid) { 
                            els.infoSavings.textContent = formatCurrency(savingsByClient[cid]); 
                            loadClientFinancialData(cid); 
                        } 
                    } 
        ...
        function initHistoryFilters() {
            const unions = getUnions('all');
            els.historyUnionFilter.innerHTML = '<option value="all">All Unions</option>';
            unions.forEach(u => els.historyUnionFilter.innerHTML += `<option value="${u}">${u}</option>`);
        }

        els.historyCoFilter.addEventListener('change', (e) => {
            state.coFilter = e.target.value;
            const unions = getUnions(state.coFilter);
            els.historyUnionFilter.innerHTML = '<option value="all">All Unions</option>';
            unions.forEach(u => els.historyUnionFilter.innerHTML += `<option value="${u}">${u}</option>`);
            state.unionFilter = 'all';
            loadTransactions(1);
        });
        ...
        document.querySelectorAll('.filter-date-btn').forEach(btn => { 
            btn.addEventListener('click', (e) => { 
                document.querySelectorAll('.filter-date-btn').forEach(b => { 
                    b.className = 'filter-date-btn text-xs px-3 py-1 text-gray-500 hover:bg-gray-100 dark:hover:bg-slate-700 transition'; 
                }); 
                e.target.className = 'filter-date-btn active text-xs px-3 py-1 bg-green-100 text-green-600 font-bold transition'; 
        ...
        function animateNumbers() { 
            document.querySelectorAll('.count-up').forEach(el => { 
                ...
                const anim = (ts) => { 
                    ...
                    el.innerText = c ? Math.round(p*t).toLocaleString() : Math.round(p*t); 
