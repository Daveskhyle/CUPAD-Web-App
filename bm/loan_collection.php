<?php
// bm/loan_collection.php - Branch Manager Version
...
if (isset($_GET['ajax_action']) && $_GET['ajax_action'] === 'get_transactions') {
    ...
    try {
        // Build Query
        $sql = "SELECT lc.transaction_id, c.name as client_name, lc.amount_collected, lc.date, lc.remaining_balance, lc.officer 
        ...
        $count_sql = str_replace("SELECT lc.transaction_id, c.name as client_name, lc.amount_collected, lc.date, lc.remaining_balance, lc.officer", "SELECT COUNT(*) as cnt", $sql);
...
// Fetch Clients & Assignments (Unions) for the entire branch
$clients =[];
$assigned_unions = [];
$unions_by_co =[];
...
try {
    $stmt = $pdo->prepare("SELECT id, name, `union`, phone, officer_username FROM clients WHERE branch_id = ? AND status = 'active' ORDER BY name ASC");
...
// Fetch Disbursements (Active Loans) for JS calculation (Branch Wide)
$disbursements =[];
try {
    $stmt = $pdo->prepare("
        SELECT d.id, d.client_id, d.officer, d.total_payable, d.remaining_balance, d.num_installments, d.date, d.status, d.loan_term_type, d.client_type 
...
// Statistics: Outstanding (Branch Wide)
$total_outstanding_loans = count($disbursements);
$total_outstanding_amount = array_sum(array_column($disbursements, 'remaining_balance'));

// Statistics: Branch Month Total
...
try {
    $stmt = $pdo->prepare("
        SELECT SUM(lc.amount_collected) as total 
        FROM loan_collections lc
        JOIN clients c ON lc.client_id = c.id
        WHERE c.branch_id = ? AND DATE(lc.date) BETWEEN ? AND ?
    ");
    $stmt->execute([$branch_id, $current_month_start, $current_month_end]);
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $branch_month_total = floatval($result['total'] ?? 0);
} catch (Exception $e) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$client_exists) {
        ...
        if ($stmt_check->fetchColumn() != $branch_id) {
            $response['message'] = 'Unauthorized client branch mismatch.';
        } elseif ($custom_pay_amount <= 0 && $installments <= 0) {
            $response['message'] = 'Invalid payment amount.';
        } else {
            // Find Active Loan - Get MOST RECENT by date
            $loan = null;
            ...
            if (!$loan) {
                ...
                // Client specific CO weekly status
                $client_co = '';
                foreach ($clients as $c) { if($c['id'] == $client_id) $client_co = $c['officer_username']; }
                ...
                $total_inst = intval($loan['num_installments'] ?? ($is_co_weekly ? 24 : 23));
                ...
                if ($custom_pay_amount <= 0 && $installments > $max_allowed) {
                    ...
                    try {
                        ...
                        if ($custom_pay_amount > 0) {
                            if (!$collection_settings['allow_partial_payments'] && !$collection_settings['allow_overpayment']) {
                                 // Strict check could go here, but usually custom overrides partial check in UI
                            }
                        ...
                        // Insert Collection (Logged by BM)
...
<body>
    ...
    <main class="max-w-7xl mx-auto px-4 py-6">
        ...
        <div class="mb-6">
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400">Manage branch repayments and view history</p>
        </div>
        ...
        <div class="dashboard-grid">
            <div class="dashboard-card card-gradient-purple">
                ...
                <div class="flex justify-between items-start relative z-10">
                    <div>
                        <div class="text-sm font-semibold uppercase opacity-90">Branch Active Loans</div>
                        <div class="text-3xl font-extrabold mt-1 mb-2 count-up" data-target="<?php echo $total_outstanding_loans; ?>">0</div>
                        <div class="text-xs bg-black/20 rounded-full px-3 py-1 inline-flex items-center"><i class="fas fa-exclamation-circle mr-1"></i> Outstanding</div>
                    </div>
            ...
            <div class="dashboard-card card-gradient-amber">
                ...
                <div class="flex justify-between items-start relative z-10">
                    <div>
                        <div class="text-sm font-semibold uppercase opacity-90">Branch Balance Due</div>
            ...
            <div class="dashboard-card card-gradient-blue">
                ...
                <div class="flex justify-between items-start relative z-10">
                    <div>
                        ...
                        <div class="text-3xl font-extrabold mt-1 mb-2">₦<span class="count-up" data-target="<?php echo $branch_month_total; ?>" data-currency="true">0</span></div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <form id="collectionForm" class="p-5 space-y-5">
                        ...
                        <div id="installment-section" class="hidden">
                            ...
                            <div class="input-group py-2">
                                ...
                                <select id="num_installments" name="num_installments" class="input-field cursor-pointer pl-8">
            ...
            <div class="lg:col-span-2">
                <!-- Changed: Fixed height, overflow-hidden to contain scrollbar inside -->
                <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-700 h-[650px] flex flex-col overflow-hidden relative">
                    ...
                    <div class="p-4 border-b border-gray-100 dark:border-slate-700 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 shrink-0">
                        ...
                        <div class="flex gap-2 overflow-x-auto w-full sm:w-auto pb-1 sm:pb-0" style="scrollbar-width: none; -ms-overflow-style: none;">
                            <select id="history-co-filter" class="text-xs bg-gray-100 dark:bg-slate-700 border-none rounded-lg px-2 py-1 outline-none shrink-0 cursor-pointer font-semibold text-purple-600">
                    ...
                    <!-- Body: Flex-1 to fill remaining space, overflow-y-auto for internal scrolling -->
    ...
    <script>
        ...
        // PURPLE THEME COLOR for Activity Feed
        ...
        let state = {
            ...
            coFilter: 'all',
            unionFilter: 'all',
            dateFilter: 'current_month', // Default to current_month
            currentPage: 1,
            limit: 10,
            isLoading: false
        };
        ...
        function selectUnion(union, btnEl) {
            ...
            if(btnEl) btnEl.className = 'px-3 py-1 rounded-full text-xs font-bold border transition bg-purple-600 text-white border-purple-600';
        ...
        function populateClients(union, co) {
            ...
            if (union) {
                const filteredClients = getClients(union, co);
        ...
        function getActiveLoan(clientId) {
            // Branch level - just match the client_id, not the officer
            const clientDisbursements = disbursements
                .filter(d => (d.client_id === clientId))
                .sort((a, b) => new Date(b.date) - new Date(a.date));
            return clientDisbursements.find(d => (d.remaining_balance === undefined || d.remaining_balance > 0.01) && d.status !== 'completed');
        }
        ...
        els.clientSelect.addEventListener('change', (e) => {
            ...
            if (loan) {
                ...
                // Get CO Weekly status for this specific client
                let clientCO = '';
                allClientsData.forEach(c => { if(c.id == clientId) clientCO = c.officer_username; });
                const isWeekly = coWeeklyStatus[clientCO] || false;

                const numInst = parseInt(loan.num_installments) || (isWeekly ? 24 : 23);
                ...
                setupInstallments(remaining, instAmt, numInst);
            } else {
        ...
        function setupInstallments(remainingBal, amountPerInst, totalInstallments) {
                ...
                <div class="activity-card group hover:bg-gray-50 dark:hover:bg-slate-700/50 transition">
                    ...
                    <div class="ac-content min-w-0"> 
                        ...
                        <div class="ac-bottom">
                            ...
                            <div class="activity-date whitespace-nowrap"><i class="fas fa-user-tie"></i> ${item.officer} &bull; <i class="far fa-clock mr-1"></i>${item.date.substring(5,16)}</div>
                        </div>
                    </div>
                </div>`;
        ...
        els.form.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (selectedValue === 'custom') {
                    if (!settings.allow_partial_payments) {
                        showToast('Custom amounts are not allowed', 'error'); els.submitBtn.disabled=false; els.submitBtn.innerHTML='Record Payment'; return;
                    }
                    const customAmount = parseFloat(document.getElementById('custom-amount')?.value || '0');
                    if (customAmount <= 0) {
                        showToast('Please enter a valid amount', 'error'); els.submitBtn.disabled=false; els.submitBtn.innerHTML='Record Payment'; return;
                    }
                    ...
                    if (loan) {
                        ...
                        if (!settings.allow_overpayment && (customAmount - remainingBal) > 0.01) {
                            showToast(`Amount exceeds remaining balance of ${formatCurrency(remainingBal)}`, 'error'); els.submitBtn.disabled=false; els.submitBtn.innerHTML='Record Payment'; return;
                        }
                ...
                if (result.success) {
                    ...
                    hideInfo();
                    populateClients(state.currentUnion, els.coFilter.value);
                    loadTransactions(1);
                    
                    // Increment Stats dynamically
                    document.querySelector('.card-gradient-blue .count-up').dataset.target = parseFloat(document.querySelector('.card-gradient-blue .count-up').dataset.target) + parseFloat(result.transaction.amount_collected);
                    document.querySelector('.card-gradient-amber .count-up').dataset.target = parseFloat(document.querySelector('.card-gradient-amber .count-up').dataset.target) - parseFloat(result.transaction.amount_collected);
                    if (result.transaction.loan_paid_off) {
                        document.querySelector('.card-gradient-purple .count-up').dataset.target = parseInt(document.querySelector('.card-gradient-purple .count-up').dataset.target) - 1;
                    }
                    animateNumbers();
