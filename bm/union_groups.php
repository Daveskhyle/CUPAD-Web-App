...
// bm/union_groups.php
// Branch Manager: Assign union/group to branch/CO and view all branch unions/groups
session_start();
...
$base_path = '../';
$page_title = "Branch Unions & Groups";

// 3. USER DETAILS
$user_id = $_SESSION['user_id'];
$username = $_SESSION['username'] ?? '';

// Fetch BM's Branch ID & Profile
$stmt_b = $pdo->prepare("SELECT branch_id, full_name, profile_pic FROM users WHERE id = ?");
...
$stmt->execute([$bm_branch_id]);
$bm_branch_name = $stmt->fetchColumn() ?: 'Unknown';

// Fetch Active Credit Officers in this Branch
$stmt = $pdo->prepare("SELECT username, full_name FROM users WHERE branch_id = ? AND role = 'co' AND status = 'active'");
$stmt->execute([$bm_branch_id]);
$branch_cos = $stmt->fetchAll(PDO::FETCH_ASSOC);
$co_lookup = [];
foreach ($branch_cos as $co) {
    $co_lookup[$co['username']] = $co['full_name'] ?: $co['username'];
}
...
// Handle Form Submit (BM creates on behalf of CO)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_union'])) {
    ...
    $assigned_co = trim($_POST['assigned_co'] ?? '');
    $assigned_branch = $bm_branch_name;

    if ($new_name && $assigned_co && $assigned_branch && $assigned_branch !== 'Unknown') {
        // Validate that assigned_co actually belongs to this branch
        if (!array_key_exists($assigned_co, $co_lookup)) {
            $_SESSION['error_message'] = "Invalid Credit Officer selected.";
        } else {
            $pdo->beginTransaction();
            try {
                // Check if the union/group already exists in the `unions` table globally
                ...
                if (!$duplicate) {
                    ...
                    $_SESSION['success_message'] = "Group '$new_name' successfully assigned to " . htmlspecialchars($co_lookup[$assigned_co]) . ".";
                } else {
                    $_SESSION['error_message'] = "This group is already assigned to this Credit Officer.";
                }
...
// Fetch Official Assignments for the ENTIRE Branch
$branch_assignments =[];
if ($bm_branch_name !== 'Unknown') {
    $stmt = $pdo->prepare("SELECT `union`, co FROM assignments WHERE branch = ? AND status = 'active'");
    $stmt->execute([$bm_branch_name]);
    $branch_assignments = $stmt->fetchAll(PDO::FETCH_ASSOC);
}
$assigned_union_names = array_column($branch_assignments, 'union');
$union_to_co_map =[];
foreach ($branch_assignments as $assignment) {
    // If a union has multiple COs, we map it as an array
    $union_to_co_map[$assignment['union']][] = $co_lookup[$assignment['co']] ?? $assignment['co'];
}

// Fetch ALL Client Data in this Branch
$clients_by_union =[];
$total_clients_managed = 0;

try {
    $stmt = $pdo->prepare("
        SELECT c.id, c.name, c.phone, c.union, c.officer_username 
        FROM clients c 
...
// Merge and unique to get the full list for tabs (clients unions + officially assigned unions)
$unions_with_clients = array_keys($clients_by_union);
$unique_assigned_unions = array_unique(array_merge($unions_with_clients, $assigned_union_names));
...
<head>
    ...
    <title>Unions & Groups | CUPAD BM</title>
    ...
    <style>
        ...
        .form-input { width: 100%; padding: 0.75rem 1rem; border-radius: 0.75rem; border: 1px solid var(--border-color); background: var(--bg-primary); color: var(--text-primary); transition: all 0.2s; outline: none; }
        .form-input:focus { border-color: var(--primary-color); box-shadow: 0 0 0 2px rgba(59, 130, 246, 0.2); }
...
<body>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex justify-between items-center mb-6">
            <div>
                <h1 class="text-2xl font-extrabold text-gray-900 dark:text-white">Branch Unions & Groups</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400">Manage client assignments and groups for <strong><?= htmlspecialchars($bm_branch_name) ?></strong></p>
            </div>
        </div>
        ...
        <?php if (isset($_SESSION['success_message'])): ?>
            <div class="mb-4 p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl text-green-600 dark:text-green-400">
        ...
        <div class="dashboard-grid">
            <div class="dashboard-card card-gradient-blue">
                ...
                <div class="relative z-10">
                    <div class="text-sm font-semibold uppercase opacity-90">Branch Groups</div>
            ...
            <div class="lg:col-span-1">
                <div class="union-card sticky top-24">
                    <div class="union-head">
                        <h2 class="text-lg font-bold text-gray-900 dark:text-white"><i class="fas fa-plus-circle text-primary"></i> Create & Assign</h2>
                    </div>
                    ...
                    <form method="post" class="space-y-4">
                        ...
                        <div class="input-group">
                            <label class="input-label">Assign To Credit Officer</label>
                            <select name="assigned_co" required class="form-input">
                                <option value="">Select Officer...</option>
                                <?php foreach ($branch_cos as $co): ?>
                                    <option value="<?= htmlspecialchars($co['username']) ?>">
                                        <?= htmlspecialchars($co['full_name'] ?: $co['username']) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        ...
                        <div class="pt-4 border-t border-gray-100 dark:border-gray-700">
                            <div>
                                <label class="text-xs text-gray-500">Branch Context</label>
                                <input type="text" value="<?= htmlspecialchars($bm_branch_name) ?>" readonly class="w-full bg-gray-100 dark:bg-gray-800 border-0 rounded text-sm px-3 py-2 text-gray-500 cursor-not-allowed mt-1">
                            </div>
                        </div>
            ...
            <div class="lg:col-span-2">
                <div class="union-card min-h-[500px]">
                    <div class="union-head flex-col sm:flex-row gap-4 items-start sm:items-center">
                        <h2 class="text-lg font-bold text-gray-900 dark:text-white">Branch Groups Directory</h2>
                    </div>

                    <?php if (empty($unique_assigned_unions)): ?>
                        <div class="text-center py-12 text-gray-500">
                            <i class="fas fa-folder-open text-4xl mb-3 opacity-30"></i>
                            <p>No groups found in this branch.</p>
                        </div>
                    <?php else: ?>
                        <!-- Tabs -->
                        <div class="tabs-container">
                            <?php foreach ($unique_assigned_unions as $i => $union_name): 
                                ...
                                // Identify who "owns" this group for tab display
                                $assigned_cos = $union_to_co_map[$union_name] ??[];
                                if (!empty($assigned_cos)) {
                                    $co_display_name = explode(' ', trim($assigned_cos[0]))[0]; 
                                } elseif (!empty($clients_by_union[$union_name])) {
                                    // Fallback to the officer of the first client in this group
                                    $fallback_username = $clients_by_union[$union_name][0]['officer_username'];
                                    $co_display_name = explode(' ', trim($co_lookup[$fallback_username] ?? $fallback_username))[0];
                                } else {
                                    $co_display_name = 'Unassigned';
                                }
                            ?>
                                <button onclick="switchTab('union-<?= $i ?>', this)" class="tab-btn <?= $i === 0 ? 'active' : 'inactive' ?>">
                                    <?= htmlspecialchars($union_name) ?> 
                                    <span class="ml-1 opacity-80 text-xs font-normal whitespace-nowrap">
                                        &bull; <i class="fas fa-user-tie"></i> <?= htmlspecialchars($co_display_name) ?> (<?= $count ?>)
                                    </span>
                                </button>
                            <?php endforeach; ?>
                        </div>
                        ...
                        <?php foreach ($unique_assigned_unions as $i => $union_name): 
                            ...
                            // Find Assigned COs for this group
                            $assigned_cos = $union_to_co_map[$union_name] ??[];
                            if (!empty($assigned_cos)) {
                                $assigned_co_text = implode(', ', $assigned_cos);
                            } elseif (!empty($union_clients)) {
                                // Fallback to the officer of the first client in this group
                                $fallback_username = $union_clients[0]['officer_username'];
                                $assigned_co_text = $co_lookup[$fallback_username] ?? $fallback_username;
                            } else {
                                $assigned_co_text = 'Unassigned';
                            }
                        ?>
                            <div id="union-<?= $i ?>" class="tab-content <?= $i !== 0 ? 'hidden' : '' ?>">
                                <div class="mb-4 flex items-center justify-between bg-gray-50 dark:bg-gray-800 p-3 rounded-lg border border-gray-100 dark:border-gray-700">
                                    <div class="flex flex-col">
                                        ...
                                        <span class="text-xs font-semibold text-gray-700 dark:text-gray-300 mt-1"><i class="fas fa-user-tie text-primary mr-1"></i> Officer: <?= htmlspecialchars($assigned_co_text) ?></span>
                                    </div>
                                    <span class="text-xs font-bold text-primary px-3 py-1 bg-blue-100 dark:bg-blue-900/30 rounded-full"><?= count($union_clients) ?> Clients</span>
                                </div>
                                    ...
                                    <table class="w-full">
                                        <thead>
                                            <tr>
                                                <th>Client Name</th>
                                                <th>Contact</th>
                                                <th class="hidden sm:table-cell">Reg. Officer</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            ...
                                            <?php else: ?>
                                                <?php foreach ($union_clients as $client): 
                                                    $client_co_name = $co_lookup[$client['officer_username']] ?? $client['officer_username'];
                                                ?>
                                                    <tr class="hover:bg-gray-50 dark:hover:bg-white/5 transition">
                                                        <td>
                                                            <div class="flex items-center gap-3">
                                                                <div class="w-8 h-8 rounded-full bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-300 flex items-center justify-center text-xs font-bold shrink-0">
                                                        ...
                                                        <td class="hidden sm:table-cell">
                                                            <span class="text-xs px-2 py-1 rounded bg-gray-100 dark:bg-gray-800 text-gray-500 whitespace-nowrap">
                                                                <?= htmlspecialchars($client_co_name) ?>
                                                            </span>
                                                        </td>
                                                    </tr>
                                                <?php endforeach; ?>
