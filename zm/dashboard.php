<?php
// zm/dashboard.php - Zonal Manager Dashboard
session_start();
...
if (isset($_GET['action'])) {
    ...
    switch($action) {
        case 'load_activities':
            handleLoadActivities($pdo, $zone_id);
            break;
        case 'search_activities':
            handleSearchActivities($pdo, $zone_id);
            break;
...
function handleLoadActivities($pdo, $zone_id) {
    $page = isset($_GET['page']) ? (int)$_GET['page'] : 1;
    $limit = isset($_GET['limit']) ? (int)$_GET['limit'] : 10;
    $offset = ($page - 1) * $limit;
    $filter = $_GET['filter'] ?? 'all';

    // Get all CO usernames in this zone
    $stmt_cos = $pdo->prepare("
        SELECT u.username 
        FROM users u
        JOIN branches b ON u.branch_id = b.id
        JOIN areas a ON b.area_id = a.id
        WHERE a.zone_id = :zid AND u.role = 'co' AND u.status = 'active'
    ");
    $stmt_cos->execute(['zid' => $zone_id]);
...
function handleSearchActivities($pdo, $zone_id) {
    $query = $_GET['q'] ?? '';
    $searchTerm = "%$query%";
    $filter = $_GET['filter'] ?? 'all';
...
$sql_user = "
    SELECT u.full_name, u.profile_pic, z.name as zone_name
    FROM users u
...
// --- STATS CALCULATION (Zone Level) ---
$start_of_month = date('Y-m-01');
$today = date('Y-m-d');

// 2. Zone Monthly Net Savings
$sql_net_savings = "
    ...
    JOIN areas a ON b.area_id = a.id
    WHERE a.zone_id = :zid AND CAST(s.date AS DATE) BETWEEN :start_date AND :end_date";
$stmt_ns = $pdo->prepare($sql_net_savings);
$stmt_ns->execute(['zid' => $zone_id, 'start_date' => $start_of_month, 'end_date' => $today]);
$zone_monthly_savings = $stmt_ns->fetchColumn();

// 3. Zone Monthly Disbursement
$sql_disb = "
    ...
    JOIN branches b ON c.branch_id = b.id
    JOIN areas a ON b.area_id = a.id
    WHERE a.zone_id = :zid AND CAST(d.created_at AS DATE) BETWEEN :start_date AND :end_date";
$stmt_d = $pdo->prepare($sql_disb);
$stmt_d->execute(['zid' => $zone_id, 'start_date' => $start_of_month, 'end_date' => $today]);
$zone_monthly_disbursed = $stmt_d->fetchColumn();

// 4. Zone Active Loans Count & Outstanding Balance (derived from area data after query #6)

// 5. Total Zone Active Clients
$sql_clients = "
    SELECT COUNT(*) 
    FROM clients c
    JOIN branches b ON c.branch_id = b.id
    JOIN areas a ON b.area_id = a.id
    WHERE a.zone_id = :zid AND c.status = 'active'";
$stmt_c = $pdo->prepare($sql_clients);
$stmt_c->execute(['zid' => $zone_id]);
$zone_total_clients = $stmt_c->fetchColumn();

// 6. Area Performance List
$sql_area_perf = "
    SELECT 
        a.id, 
        a.name as area_name,
        ...
        (SELECT COALESCE(SUM(CASE WHEN amount < 0 OR LOWER(s.type) IN ('withdrawal', 'return', 'adjust') THEN -ABS(amount) ELSE amount END), 0) 
         FROM saving_collections s JOIN clients c2 ON s.client_id = c2.id JOIN branches b2 ON c2.branch_id = b2.id
         WHERE b2.area_id = a.id AND CAST(s.date AS DATE) BETWEEN :start_date1 AND :end_date1) as monthly_net_savings,
        (SELECT COALESCE(SUM(lc.amount_collected), 0) 
         FROM loan_collections lc JOIN clients c3 ON lc.client_id = c3.id JOIN branches b3 ON c3.branch_id = b3.id
         WHERE b3.area_id = a.id AND CAST(lc.date AS DATE) BETWEEN :start_date2 AND :end_date2) as monthly_collections,
        (SELECT COALESCE(SUM(d.remaining_balance), 0) 
         FROM disbursements d JOIN clients c4 ON d.client_id = c4.id JOIN branches b4 ON c4.branch_id = b4.id
         WHERE b4.area_id = a.id AND d.remaining_balance > 0) as outstanding_portfolio,
        (SELECT COUNT(*)
         FROM disbursements d JOIN clients c4 ON d.client_id = c4.id JOIN branches b4 ON c4.branch_id = b4.id
         WHERE b4.area_id = a.id AND d.remaining_balance > 0) as active_loans_count
    FROM areas a
    WHERE a.zone_id = :zid
    ORDER BY monthly_collections DESC, monthly_net_savings DESC
";
$stmt_ap = $pdo->prepare($sql_area_perf);
$stmt_ap->execute([
    'zid' => $zone_id, 
...
$area_performances = $stmt_ap->fetchAll();

// Derive zone totals from area data so stat card always matches the overview sum
$zone_outstanding_balance = array_sum(array_column($area_performances, 'outstanding_portfolio'));
$zone_active_loans_count = array_sum(array_column($area_performances, 'active_loans_count'));

// 7. Initial Activity Load
$stmt_cos_init = $pdo->prepare("
    SELECT u.username 
    FROM users u
    JOIN branches b ON u.branch_id = b.id
    JOIN areas a ON b.area_id = a.id
    WHERE a.zone_id = :zid AND u.role = 'co' AND u.status = 'active'
");
$stmt_cos_init->execute(['zid' => $zone_id]);
$co_usernames = $stmt_cos_init->fetchAll(PDO::FETCH_COLUMN);
...
<head>
    ...
    <title>Zonal Manager Dashboard | CUPAD</title>
    ...
    <main class="max-w-7xl mx-auto px-4 py-6">
        ...
        <div class="mb-6 text-center">
            ...
            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                ...
                Zonal Manager, <?php echo htmlspecialchars($zone_name); ?> Zone
            </p>
        </div>
        ...
        <div class="dashboard-grid">
            <div class="dashboard-card card-gradient-blue">
                ...
                <div class="flex justify-between items-start relative z-10">
                    <div class="min-w-0">
                        ...
                        <div class="text-2xl lg:text-3xl font-extrabold mt-1 mb-2 truncate" title="₦<?php echo number_format($zone_monthly_savings); ?>">₦<?php echo number_format($zone_monthly_savings); ?></div>
            ...
            <div class="dashboard-card card-gradient-orange">
                ...
                <div class="flex justify-between items-start relative z-10">
                    <div class="min-w-0">
                        <div class="text-sm font-semibold uppercase opacity-90">Zone Disbursed</div>
                        <div class="text-2xl lg:text-3xl font-extrabold mt-1 mb-2 truncate" title="₦<?php echo number_format($zone_monthly_disbursed); ?>">₦<?php echo number_format($zone_monthly_disbursed); ?></div>
            ...
            <div class="dashboard-card card-gradient-green">
                ...
                <div class="flex justify-between items-start relative z-10">
                    <div class="min-w-0">
                        ...
                        <div class="text-2xl lg:text-3xl font-extrabold mt-1 mb-2 truncate" title="₦<?php echo number_format($zone_outstanding_balance); ?>">
                            ₦<?php echo number_format($zone_outstanding_balance); ?>
                        </div>
                        <div class="text-xs bg-black/20 rounded-full px-3 py-1 inline-flex items-center">
                            <i class="fas fa-file-signature mr-1"></i> <?php echo number_format($zone_active_loans_count); ?> Active Loans
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="dashboard-card card-gradient-purple">
                ...
                <div class="flex justify-between items-start relative z-10">
                    <div class="min-w-0">
                        ...
                        <div class="text-2xl lg:text-3xl font-extrabold mt-1 mb-2 truncate"><?php echo number_format($zone_total_clients); ?></div>
                        <div class="text-xs bg-black/20 rounded-full px-3 py-1 inline-flex items-center truncate"><i class="fas fa-users mr-1"></i> Zone-wide Active</div>
                    </div>
                </div>
            </div>
        </div>

        <!-- QUICK ACTIONS FOR ZM -->
        <h2 class="text-lg font-bold text-gray-800 dark:text-white mb-3">Zone Management</h2>
        ...
        <!-- AREAS PERFORMANCE (Zone Level) -->
        <h2 class="text-lg font-bold text-gray-800 dark:text-white mb-3">Areas Overview</h2>
        <div class="co-scroll-container">
            <?php foreach ($area_performances as $area): ?>
            <div class="co-card">
                <div class="co-head">
                    <div class="co-name" title="<?php echo htmlspecialchars($area['area_name']); ?>">
                        <i class="fas fa-map-marked-alt mr-1 text-pink-500"></i> <?php echo htmlspecialchars($area['area_name']); ?>
                    </div>
                    <div class="co-count"><i class="fas fa-users"></i> <?php echo number_format($area['active_clients']); ?></div>
                </div>
                <div class="co-stat-row">
                    <span style="color:var(--text-secondary);">Monthly Net Savings</span>
                    <span style="color:var(--success-color); font-weight:700;">₦<?php echo number_format($area['monthly_net_savings']); ?></span>
                </div>
                <div class="co-stat-row">
                    <span style="color:var(--text-secondary);">Monthly Collections</span>
                    <span style="color:var(--primary-color); font-weight:700;">₦<?php echo number_format($area['monthly_collections']); ?></span>
                </div>
                <div class="co-stat-row" style="margin-top: 0.5rem; border-top: 1px dashed var(--border-color); padding-top: 0.5rem;">
                    ...
                    <span style="color:var(--warning-color); font-weight:700;">₦<?php echo number_format($area['outstanding_portfolio']); ?></span>
                </div>
            </div>
            <?php endforeach; ?>
            <?php if(empty($area_performances)): ?>
                <div class="text-gray-400 italic p-4">No areas assigned to this zone yet.</div>
            <?php endif; ?>
        </div>

        <!-- ZONE WIDE SEARCH & FEED -->
        ...
        <div class="activities-container">
            <div class="flex justify-between mb-4">
                <h2 class="text-lg font-bold text-gray-800 dark:text-white">Zone Activity Feed</h2>
            ...
            <div id="activities-list">
                ...
                <?php if(empty($initial_activities)): ?>
                    <div class="text-center py-8 text-gray-400">No zone activities found.</div>
                <?php endif; ?>
            </div>
    ...
    <script>
        class EnhancedDashboard {
            ...
            showBrowserNotification(message) {
                if ("Notification" in window && Notification.permission === "granted") {
                    new Notification("CUPAD ZM Dashboard", {
                        body: message,
