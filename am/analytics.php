<?php
// am/analytics.php - Area Manager Analytics Dashboard
session_start();
...
$am_username = $_SESSION['username'] ?? '';
$user_id = $_SESSION['user_id'] ?? '';

// Fetch the AM's area_id and details
$stmt_a = $conn->prepare("SELECT u.area_id, a.name as area_name, u.full_name, u.profile_pic FROM users u LEFT JOIN areas a ON u.area_id = a.id WHERE u.id = ?");
$stmt_a->execute([$user_id]);
$am_info = $stmt_a->fetch(PDO::FETCH_ASSOC);

$area_id = $am_info['area_id'] ?? '';
$area_name = $am_info['area_name'] ?? 'My Area';
$full_name = !empty($am_info['full_name']) ? $am_info['full_name'] : 'Area Manager';
$profile_pic = !empty($am_info['profile_pic']) ? $am_info['profile_pic'] : 'default_avatar.png';

// Fetch all active branches in this area
$branch_list =[];
$stmt_branches = $conn->prepare("SELECT id, name FROM branches WHERE area_id = ? AND status = 'active' ORDER BY name ASC");
$stmt_branches->execute([$area_id]);
...
// --- HYBRID CACHE SYSTEM ---
function get_cache_key($area_id, $date_from, $date_to, $search = '') {
    return md5($area_id . $date_from . $date_to . $search . 'v_am_analytics_v1');
}
...
$cache_key = get_cache_key($area_id, $req_date_from, $req_date_to);
$cached_data = get_cached_data($cache_key, $cache_dir); 

if (isset($_GET['force']) && $_GET['force'] == '1') $cached_data = null; // Force refresh

if ($cached_data) {
    $area_clients = $cached_data['area_clients'];
    $area_savings = $cached_data['area_savings'];
    $area_disbursements = $cached_data['area_disbursements'];
    $area_collections = $cached_data['area_collections'];
    $area_registrations = $cached_data['area_registrations'];
} else {
    ...
    if (!empty($branch_ids)) {
        ...
        $area_clients = $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
    ...
    if (!empty($branch_ids)) {
        ...
        $stmt->execute($branch_ids);
        $area_savings = $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
    ...
    if (!empty($branch_ids)) {
        ...
        $stmt->execute($branch_ids);
        $area_disbursements = $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
    ...
    if (!empty($branch_ids)) {
        ...
        $stmt->execute($branch_ids);
        $area_collections = $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
    ...
    if (!empty($branch_ids)) {
        ...
        $stmt->execute($branch_ids);
        $client_map = array_column($area_clients, 'name', 'id');
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $row['client_name'] = $client_map[$row['client_id']] ?? 'Unknown';
            $area_registrations[] = $row;
        }
    }
    
    save_to_cache([
        'area_clients' => $area_clients,
        'area_savings' => $area_savings,
        'area_disbursements' => $area_disbursements,
        'area_collections' => $area_collections,
        'area_registrations' => $area_registrations
    ], $cache_key, $cache_dir);
}
...
// Apply Filters
$filtered_savings = filter_by_date_range($area_savings, $date_from, $date_to);
$filtered_disbursements = filter_by_date_range($area_disbursements, $date_from, $date_to);
$filtered_collections = filter_by_date_range($area_collections, $date_from, $date_to);
$filtered_registrations = filter_by_date_range($area_registrations, $date_from, $date_to);

// --- 5. METRICS CALCULATION (AREA WIDE) ---
$total_clients = count(array_filter($area_clients, fn($c) => $c['status'] === 'active'));
...
foreach ($area_disbursements as $loan) {
...
if (isset($_GET['ajax']) && $_GET['ajax'] === '1') {
    ...
    // 6a. Main AJAX call - Area Dash & Branch Table
    if ($action === 'main') {
        $branch_stats = [];
        foreach ($branch_list as $branch) {
            $b_id = $branch['id'];
            $b_clients = array_filter($area_clients, fn($c) => $c['branch_id'] == $b_id);
            ...
            $all_branch_loans = array_filter($area_disbursements, fn($d) => in_array($d['client_id'], $b_client_ids));
        ...
        echo json_encode([
            ...
            'main_table' => $branch_stats,
            'dates' => ['from' => $date_from, 'to' => $date_to]
        ]);
        exit();
    }

    // 6b. Branch Details (CO Breakdown)
    if ($action === 'branch_details') {
        ...
        foreach ($branch_cos as $co) {
            $co_user = $co['username'];
            $officer_clients = array_filter($area_clients, fn($c) => $c['officer_username'] === $co_user);
            ...
            $paid_off_count = 0; $overdue_count = 0;
            $all_officer_loans = array_filter($area_disbursements, fn($d) => $d['officer'] === $co_user);
    ...
    // 6c. CO Details (Union Breakdown)
    if ($action === 'co_details') {
        $co_username = $_GET['co_username'] ?? '';
        $officer_clients = array_filter($area_clients, fn($c) => $c['officer_username'] === $co_username);
        $officer_disbursements_all = array_filter($area_disbursements, fn($d) => $d['officer'] === $co_username);
        
        $ajax_filtered_collections = array_filter($area_collections, fn($c) => $c['officer'] === $co_username && $c['date'] >= $aj_from && $c['date'] <= $aj_to." 23:59:59");
        $ajax_filtered_savings = array_filter($area_savings, fn($s) => $s['officer'] === $co_username && $s['date'] >= $aj_from && $s['date'] <= $aj_to." 23:59:59");
        $ajax_filtered_registrations = array_filter($area_registrations, fn($r) => $r['officer'] === $co_username && $r['date'] >= $aj_from && $r['date'] <= $aj_to." 23:59:59");
    ...
    // 6d. Union Details (Client Breakdown)
    if ($action === 'union_details') {
        $co_username = $_GET['co_username'] ?? '';
        $union_name = $_GET['union_name'] ?? '';
        $client_search = isset($_GET['q']) ? strtolower(trim($_GET['q'])) : '';

        $co_savings_all = array_filter($area_savings, fn($s) => $s['officer'] === $co_username);
        $co_disbursements_all = array_filter($area_disbursements, fn($d) => $d['officer'] === $co_username);
        $ajax_filtered_collections = array_filter($area_collections, fn($c) => $c['officer'] === $co_username && $c['date'] >= $aj_from && $c['date'] <= $aj_to." 23:59:59");
        $ajax_filtered_savings = array_filter($area_savings, fn($s) => $s['officer'] === $co_username && $s['date'] >= $aj_from && $s['date'] <= $aj_to." 23:59:59");
        $ajax_filtered_disbursements = array_filter($area_disbursements, fn($d) => $d['officer'] === $co_username && $d['date'] >= $aj_from && $d['date'] <= $aj_to." 23:59:59");
        
        $target_clients = array_filter($area_clients, fn($c) => $c['officer_username'] === $co_username && $c['union'] === $union_name);
...
<head>
    ...
    <title>Area Analytics | CUPAD</title>
...
<body>
    ...
    <header class="main-header">
        <nav class="navbar">
            <a href="dashboard.php" class="flex items-center gap-2 no-underline">
                ...
                <span class="text-xl font-extrabold text-blue-600 tracking-tight">CUPAD AM</span>
            </a>
    ...
    <main class="max-w-7xl mx-auto px-4 py-6">
        ...
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6">
            <div>
                <h1 class="text-2xl font-extrabold text-gray-900 dark:text-white">Area Analytics</h1>
                <div class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mt-1">
                    <span><i class="fas fa-map-marked-alt text-blue-500"></i> <?php echo htmlspecialchars($area_name); ?></span>
                    <span>&bull;</span>
        ...
        <div class="dashboard-grid">
            <div class="dashboard-card card-gradient-purple"> 
                ...
                <div class="flex justify-between items-start relative z-10"> 
                    <div>
                        <div class="text-sm font-semibold uppercase opacity-90">Area Active Clients</div> 
                        <div class="text-3xl font-extrabold mt-1 mb-2 count-up" id="metric_clients">0</div> 
                        <div class="text-xs bg-black/20 rounded-full px-3 py-1 inline-flex items-center"><i class="fas fa-globe mr-1"></i> Across All Branches</div> 
                    </div>
            ...
            <div class="dashboard-card card-gradient-orange"> 
                ...
                <div class="flex justify-between items-start relative z-10"> 
                    <div>
                        <div class="text-sm font-semibold uppercase opacity-90">Area Disbursements</div> 
            ...
            <div class="dashboard-card card-gradient-green"> 
                ...
                <div class="flex justify-between items-start relative z-10"> 
                    <div>
                        <div class="text-sm font-semibold uppercase opacity-90">Area Collections</div> 
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
            <div class="bg-white dark:bg-gray-800 rounded-2xl p-5 border border-gray-200 dark:border-gray-700 shadow-sm"> 
                <h3 class="font-bold text-gray-800 dark:text-white mb-4 flex items-center gap-2"><i class="fas fa-heartbeat text-error"></i> Area Portfolio Health</h3> 
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl p-5 border border-gray-200 dark:border-gray-700 shadow-sm"> 
                <h3 class="font-bold text-gray-800 dark:text-white mb-4 flex items-center gap-2"><i class="fas fa-calculator text-primary"></i> Area Financials</h3> 
                <div class="space-y-3"> 
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl p-5 border border-gray-200 dark:border-gray-700 shadow-sm flex flex-col"> 
                <h3 class="font-bold text-gray-800 dark:text-white mb-2 flex items-center gap-2"><i class="fas fa-chart-pie text-secondary"></i> Area Portfolio Mix</h3> 
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 mb-8 border border-gray-200 dark:border-gray-700 shadow-sm">
            <h3 class="font-bold text-gray-800 dark:text-white mb-4 flex items-center gap-2"><i class="fas fa-chart-line text-primary"></i> Area Activity Trend</h3>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
            <div class="p-4 border-b border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row justify-between items-center gap-3">
                 <h3 class="font-bold text-lg text-gray-800 dark:text-white">Performance by Branch</h3>
                 <div class="relative w-full sm:w-64">
                    <i class="fas fa-search absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"></i>
                    <input type="text" id="mainSearchInput" placeholder="Search branches..." class="w-full pl-9 py-2 bg-gray-100 dark:bg-gray-900 border-none rounded-lg text-sm text-gray-700 dark:text-gray-300 focus:ring-2 focus:ring-primary">
                </div>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm whitespace-nowrap">
                    <thead class="bg-gray-50 dark:bg-gray-900/50 text-gray-500 dark:text-gray-400 uppercase text-xs font-semibold">
                        <tr>
                            <th class="p-4">Branch Name</th> <th class="p-4 text-center">COs</th> <th class="p-4 text-center">Clients</th> <th class="p-4 text-right">Deposits</th> <th class="p-4 text-right">Withdrawals</th> <th class="p-4 text-right">Net Savings</th> <th class="p-4 text-right">Disbursed</th> <th class="p-4 text-right">Collected</th> <th class="p-4 text-center">Reg.</th> <th class="p-4 text-center">Paid Off</th> <th class="p-4 text-center">Overdue</th>
                        </tr>
                    </thead>
                    <tbody id="mainTableBody" class="divide-y divide-gray-100 dark:divide-gray-700">
                        <tr><td colspan="11" class="p-8 text-center text-gray-500">Loading data...</td></tr>
                    </tbody>
    ...
    <script>
        ...
        function renderTable(rows) {
            ...
            if(!rows || !rows.length) {
                tbody.innerHTML = '<tr><td colspan="11" class="p-8 text-center text-gray-500">No active branches found.</td></tr>';
            ...
            const tableHtml = rows.map(r => {
                ...
                return `<tr class="data-row hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer transition" data-name="${r.name.toLowerCase()}" onclick="openBranchDetails('${r.id}', '${r.name.replace(/'/g,"\\'")}')">
                    <td class="p-4 font-medium text-blue-600 dark:text-blue-400"><i class="fas fa-building mr-1"></i> ${r.name}</td> 
                    <td class="p-4 text-center font-bold text-gray-700 dark:text-gray-300">${r.co_count}</td>
        ...
        async function openBranchDetails(branch_id, branch_name) {
            ...
            document.getElementById('modalBackButton').classList.add('hidden'); // Top level modal
        ...
        function closeModal() {
            ...
            modalState = { view: null, branch: null, officer: null, union: null };
            document.getElementById('modalSearchInput').value = '';
        }
