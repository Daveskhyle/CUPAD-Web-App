...
// am/clients.php - Area Manager Client View
session_start();
...
// Fetch AM's Area ID
$stmt_a = $pdo->prepare("SELECT area_id, full_name, profile_pic FROM users WHERE id = ?");
$stmt_a->execute([$user_id]);
$user_data = $stmt_a->fetch(PDO::FETCH_ASSOC);
$area_id = $user_data['area_id'] ?? 0;
$full_name = $user_data['full_name'] ?? 'Area Manager';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax']) && $_POST['ajax'] === 'update_guarantor') {
    ...
    try {
        // Ensure the client belongs to a branch within the AM's area
        ...
        if ($stmt->execute([$g_name, $g_phone, $client_id])) {
            if($stmt->rowCount() > 0) {
                ...
                echo json_encode(['success' => false, 'message' => 'Client not found in your area.']);
            }
        } else {
...
if (isset($_GET['ajax']) && $_GET['ajax'] === 'get_clients_data') {
    ...
    // FIX: Removed is_numeric() since branch IDs are alphanumeric strings
    $use_branch_filter = ($filter_branch !== 'all' && in_array($filter_branch, $branch_ids));
    $branch_condition = $use_branch_filter ? "c.branch_id = :filter_branch" : "c.branch_id IN ($branch_ids_str)";

    try {
        // Count Total
        $count_sql = "SELECT COUNT(id) FROM clients c WHERE $branch_condition AND c.status = 'active' AND (c.name LIKE :s1 OR c.`union` LIKE :s2)";
        ...
        // Total Savings
        $sav_sql = "SELECT SUM(sb.balance) FROM saving_balances sb JOIN clients c ON sb.client_id = c.id WHERE $branch_condition AND c.status = 'active' AND (c.name LIKE :s1 OR c.`union` LIKE :s2)";
        ...
        // Total Loans
        $loan_sql = "SELECT SUM(d.remaining_balance) FROM disbursements d JOIN clients c ON d.client_id = c.id WHERE $branch_condition AND d.remaining_balance > 0";
...
if (isset($_GET['ajax']) && $_GET['ajax'] === 'get_client_history') {
    ...
    try {
        $stmt = $pdo->prepare("SELECT c.name, c.officer_username, u.full_name as officer_name, c.branch_id, b.name as branch_name,
...
<head>
    ...
    <title>Area Clients | CUPAD AM</title>
...
<body>
    ...
    <main class="max-w-7xl mx-auto px-4 py-6">
        <div class="mb-6">
            <h1 class="text-2xl font-extrabold text-gray-900 dark:text-white tracking-tight">Area Clients</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Overview of all clients within your assigned area.</p>
        </div>
        ...
        <div class="dashboard-grid">
            <div class="dashboard-card card-gradient-blue">
                ...
                <div class="flex justify-between items-start relative z-10">
                    <div class="min-w-0">
                        ...
                        <div class="text-xs bg-black/20 rounded-full px-3 py-1 inline-flex items-center"><i class="fas fa-users mr-1"></i> Active in Area</div>
                    </div>
                </div>
            </div>
            <div class="dashboard-card card-gradient-green">
                ...
                <div class="flex justify-between items-start relative z-10">
                    <div class="min-w-0">
                        <div class="text-sm font-semibold uppercase opacity-90">Area Savings</div>
            ...
            <div class="dashboard-card card-gradient-red">
                ...
                <div class="flex justify-between items-start relative z-10">
                    <div class="min-w-0">
                        ...
                        <div class="text-xs bg-black/20 rounded-full px-3 py-1 inline-flex items-center"><i class="fas fa-file-invoice-dollar mr-1"></i> Area Remaining</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="flex flex-col md:flex-row gap-3 mb-6">
            ...
            <div class="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
                <select id="branchSelect" class="custom-select flex-1 sm:w-48" onchange="resetAndLoad()">
                    <option value="all">All Branches</option>
                    <?php foreach ($area_branches as $br): ?>
                        <option value="<?php echo htmlspecialchars($br['id']); ?>"><?php echo htmlspecialchars($br['name']); ?></option>
                    <?php endforeach; ?>
                </select>
                <select id="sortSelect" class="custom-select flex-1 sm:w-40" onchange="resetAndLoad()">
    ...
    <script>
        ...
        async function loadClients(append = false) {
            ...
            try {
                const res = await fetch(`?ajax=get_clients_data&page=${page}&search=${encodeURIComponent(search)}&sort=${sort}&branch=${branch}`);
                                    ...
                                    <div class="flex gap-3 items-center">
                                        ...
                                        <div>
                                            ...
                                            <div class="text-[10px] font-bold text-indigo-600 dark:text-indigo-400 mt-0.5 flex items-center gap-1 truncate w-40">
                                                <i class="fas fa-building"></i> ${c.branch_name}
                                            </div>
                                        </div>
                                    </div>
